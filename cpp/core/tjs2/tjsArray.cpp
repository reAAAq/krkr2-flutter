            ...
            TJS_BEGIN_NATIVE_METHOD_DECL(/* func.name */push)
                    {
                        ...
                        ((tTJSArrayObject *) objthis)->Insert(ni, param, numparams,
                                                              (tjs_int) ni->Items.size());
            ...
            TJS_BEGIN_NATIVE_METHOD_DECL(/* func.name */pop)
                    {
                        ...
                        if (ni->Items.empty()) {
                            ...
                            ((tTJSArrayObject *) objthis)->Erase(ni,
                                                                 (tjs_int) (ni->Items.size() - 1));
                        }
                ...
                for (i = arrayni->Items.begin(); i != arrayni->Items.end(); i++) {
                    ...
                    if (type == tvtObject) {
                        ...
                        if (dsp && TJS_SUCCEEDED(dsp->NativeInstanceSupport(TJS_NIS_GETINSTANCE,
                            ...
                            if (objrec) {
                                Items.push_back(
                                        tTJSVariant((iTJSDispatch2 *) NULL)); // becomes null
                            } else {
                                ...
                                if (TJS_SUCCEEDED(newobj->NativeInstanceSupport(TJS_NIS_GETINSTANCE,
                                   ...
                                   TJS_SUCCEEDED(dsp->NativeInstanceSupport(TJS_NIS_GETINSTANCE,
                                                                            TJSGetArrayClassID(),
                                                                            (iTJSNativeInstance **) &arrayni))) {
                            // array
                            bool objrec = false;
...
//---------------------------------------------------------------------------
    void tTJSArrayObject::Insert(tTJSArrayNI *ni, tTJSVariant *const *val, tjs_int numvals,
                                 tjs_int num) {
        if (num < 0) num += (tjs_int) ni->Items.size();
        if (num < 0) TJS_eTJSError(TJSRangeError);
        tjs_int count = (tjs_int) ni->Items.size();
