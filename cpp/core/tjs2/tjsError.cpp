...
void TJSGetExceptionObject(tTJS *tjs, tTJSVariant *res, tTJSVariant &msg,
    ...
    hr = clo.CreateNew(0, nullptr, nullptr, &excpobj, 1, &pmsg, clo.ObjThis);
    if (TJS_FAILED(hr))
        TJS_eTJSError(TJSExceptionNotFound);
    if (trace) {
...
//---------------------------------------------------------------------------
eTJSScriptError::tScriptBlockHolder::~tScriptBlockHolder() { Block->Release(); }
...
void TJSThrowFrom_tjs_error(tjs_error hr, const tjs_char *name) {
    ...
    default:
        if (TJS_FAILED(hr)) {
            // FIXME:
            TJS_eTJSError(
                fmt::sprintf(ttstr{TJSUnknownFailure}.AsNarrowStdString(), hr));
        }
    }
}
//---------------------------------------------------------------------------
