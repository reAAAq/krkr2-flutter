        ...
        case VM_CONST:
            ...
            if (DataArea) {
                com.printf(
                    ...
                    GetValueComment(TJS_GET_VM_REG(DataArea, CodeArea[i + 2]))
                        .AsNarrowStdString()
                        .c_str());
            }
            size = 3;
...
#undef OP1_DISASM
        ...
        case VM_CCL:
            msg.printf("ccl %%%d-%%%d", TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]),
                       TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]) + CodeArea[i + 2] -
                           1);
            size = 3;
            break;
...
#undef OP1A_DISASM
        ...
        case VM_TYPEOFD:
            ...
            msg.printf(CodeArea[i] == VM_DELD ? "deld %%%d, %%%d.*%d"
                                              : "typeofd %%%d, %%%d.*%d",
                       TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]),
                       TJS_FROM_VM_REG_ADDR(CodeArea[i + 2]),
                       TJS_FROM_VM_REG_ADDR(CodeArea[i + 3]));
        ...
        case VM_TYPEOFI:
            ...
            msg.printf(CodeArea[i] == VM_DELI ? "deli %%%d, %%%d.%%%d"
                                              : "typeofi %%%d, %%%d.%%%d",
                       TJS_FROM_VM_REG_ADDR(CodeArea[i + 1]),
                       TJS_FROM_VM_REG_ADDR(CodeArea[i + 2]),
                       TJS_FROM_VM_REG_ADDR(CodeArea[i + 3]));
...
void tTJSInterCodeContext::_output_func(const tjs_char *msg,
                                        const tjs_char *comment, tjs_int addr,
...
void tTJSInterCodeContext::_output_func_src(const tjs_char *msg,
                                            const tjs_char *name, tjs_int line,
                                            void *data) {
    ttstr buf{};
...
//---------------------------------------------------------------------------
void tTJSInterCodeContext::Disassemble(void (*output_func)(const tjs_char *msg,
                                                           void *data),
                                       void *data, tjs_int start, tjs_int end) {
    // dis-assemble
    of_data dat;
