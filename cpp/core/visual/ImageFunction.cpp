...
tjs_error TJS_INTF_METHOD tTJSNI_ImageFunction::Construct(
    ...
    if (TVPIntersectRect(&destrect, rect, clipRect)) {
        ...
        case dfAlpha:
            if (opa > 0) {
                color = TVPToActualColor(color);
                updated =
                    dst->GetBitmap()->FillColorOnAlpha(destrect, color, opa);
            } else {
        ...
        case dfAddAlpha:
            if (opa >= 0) {
                color = TVPToActualColor(color);
                updated =
                    dst->GetBitmap()->FillColorOnAddAlpha(destrect, color, opa);
            }
            break;
        case dfOpaque:
            color = TVPToActualColor(color);
