    ...
    while (!m_bAbortRequest) {
        ...
        if (!msgs.empty() && (msgs.back().priority >= priority || m_drain)) {
            ...
            *pMsg = item.message->AddRef();
            ...
            if (m_hEvent.wait_for(eventLock, std::chrono::milliseconds(
...
int CDVDMessageQueue::GetLevel() {
    ...
    int level =
        std::min(100.0, std::ceil(100.0 * m_TimeSize *
                                  (m_TimeFront - m_TimeBack) / DVD_TIME_BASE));

    // if we added lots of packets with NOPTS, make sure that the queue is not
    // signalled empty
