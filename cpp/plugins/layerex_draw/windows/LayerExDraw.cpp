...
Image *loadImage(const tjs_char *name)
{
	...
	if (filename.length()) {
		if (false && !wcschr(filename.toWString().c_str(), '>')) {// にぎりっぱになるのでやめ
			// 実ファイルが存在
...
FontInfo::setFamilyName(const tjs_char *familyName)
{
  ...
  if (forceSelfPathDraw) {
	...
	if (familyName) {
		clear();
		if (privateFontCollection) {
			fontFamily = new FontFamily(ttstr{familyName}.toWString().c_str(), privateFontCollection);
...
LayerExDraw::drawPathString(const FontInfo *font, const Appearance *app, REAL x, REAL y, const tjs_char *text)
{
  if (font->getSelfPathDraw())
	...
	// 文字列のパスを準備
	GraphicsPath path;
	path.AddString(ttstr{text}.toWString().c_str(), -1, font->fontFamily, font->style, font->emSize, PointF(x, y), StringFormat::GenericDefault());
	return drawPath(app, &path);
}
	...
	while (i != app->drawInfos.end()) {
		if (i->info) {
			if (i->type == 1) { // ブラシのみ
				...
				graphics->DrawString(wText, -1, &f, p, StringFormat::GenericDefault(), brush);
				if (metaGraphics) {
					metaGraphics->DrawString(wText, -1, &f, p, StringFormat::GenericDefault(), brush);
				}
				// 更新領域計算
				if (first) {
					graphics->MeasureString(wText, -1, &f, p, StringFormat::GenericDefault(), &rect);
					transformRect(calcTransform, rect);
					first = false;
				} else {
					RectF r;
					graphics->MeasureString(wText, -1, &f, p, StringFormat::GenericDefault(), &r);
...
LayerExDraw::measureString(const FontInfo *font, const tjs_char *text)
{
  if (font->getSelfPathDraw())
	...
	graphics->SetTextRenderingHint(textRenderingHint);
	Font f(font->fontFamily, font->emSize, font->style, UnitPixel);
	graphics->MeasureString(ttstr{text}.toWString().c_str(), -1, &f, PointF(0,0), StringFormat::GenericDefault(), &rect);
	return rect;
}
...
LayerExDraw::measureStringInternal(const FontInfo *font, const tjs_char *text)
{
    ...
    graphics->SetTextRenderingHint(textRenderingHint);
    Font f(font->fontFamily, font->emSize, font->style, UnitPixel);
    graphics->MeasureString(wText, -1, &f, PointF(0,0), StringFormat::GenericDefault(), &rect);
    CharacterRange charRange(0, INT(wcslen(wText)));
    StringFormat stringFormat = StringFormat::GenericDefault();
    stringFormat.SetMeasurableCharacterRanges(1, &charRange);
    Region region;
    graphics->MeasureCharacterRanges(wText, -1, &f, rect, &stringFormat, 1, &region);
    RectF regionBounds;
...
LayerExDraw::getTextOutline(const FontInfo *fontInfo, PointF &offset, GraphicsPath *path, ttstr text)
{
  ...
  font.lfCharSet = DEFAULT_CHARSET;
  wcscpy_s(font.lfFaceName, ttstr{fontInfo->familyName}.toWString().c_str());
