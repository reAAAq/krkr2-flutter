...
tTJS::tTJS() {
    ...
    try {
        ...
        val = tTJSVariant(dsp, nullptr);
        dsp->Release();
        Global->PropSet(TJS_MEMBERENSURE, TJS_W("Dictionary"), nullptr, &val,
                        Global);
        ...
        // Math
        {
            ...
            val = tTJSVariant(dsp, nullptr);
            dsp->Release();
            math->PropSet(TJS_MEMBERENSURE, TJS_W("RandomGenerator"), nullptr,
                          &val, math);
        }
        ...
        // RegExp
        dsp =
            TJSCreateRegExpClass(); // the body is implemented in tjsRegExp.cpp
        val = tTJSVariant(dsp, nullptr);
        dsp->Release();
...
//---------------------------------------------------------------------------
void tTJS::CompileScript(const tjs_char *script, class tTJSBinaryStream *output,
