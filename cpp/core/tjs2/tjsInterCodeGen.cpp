...
void tTJSExprNode::AddDictionaryElement(const tTJSString &name,
    ...
    Val->AsObjectClosureNoAddRef().PropSet(TJS_MEMBERENSURE, membername.c_str(),
                                           membername.GetHint(), &val, nullptr);
}
//---------------------------------------------------------------------------
...
tTJSInterCodeContext::tTJSInterCodeContext(tTJSInterCodeContext *parent,
    ...
    try {
        ...
        switch (
            ContextType) // decide variable reservation count with context type
        {
        case ctTopLevel:
...
tTJSInterCodeContext::tTJSInterCodeContext(
    ...
    tjs_int varcount, tjs_int verrescount, tjs_int maxframe, tjs_int argcount,
    tjs_int arraybase, tjs_int colbase, bool srcsorted, tSourcePos *srcPos,
    tjs_int srcPosSize, std::vector<tjs_int> &superpointer)
    : inherited(TJSGetContextHashSize(type)), Properties(nullptr) {
    inherited::CallFinalize = false;
    Parent = nullptr;
    PropGetter = PropSetter = SuperClassGetter = nullptr;
...
tjs_int tTJSInterCodeContext::PutCode(tjs_int32 num, tjs_int32 pos) {
    ...
    if (pos != -1) {
        if (PrevSourcePos != pos) {
            ...
            if (!SourcePosArray) {
                SourcePosArray =
                    (tSourcePos *)TJS_malloc(TJS_INC_SIZE * sizeof(tSourcePos));
                if (!SourcePosArray)
...
tjs_int tTJSInterCodeContext::PutData(const tTJSVariant &val) {
    ...
    if (_DataAreaSize >= _DataAreaCapa) {
        ...
        _DataArea = (tTJSVariant **)TJS_realloc(
            _DataArea, sizeof(tTJSVariant *) * (_DataAreaCapa + TJS_INC_SIZE));
        if (!_DataArea)
...
//---------------------------------------------------------------------------
int TJS_USERENTRY
tTJSInterCodeContext::tSourcePos::SortFunction(const void *a, const void *b) {
...
void tTJSInterCodeContext::RegisterFunction() {
    ...
    if (Parent->ContextType == ctFunction || Parent->ContextType == ctClass) {
        ...
        Parent->PropSet(TJS_MEMBERENSURE | TJS_IGNOREPROP, Name, nullptr, &val,
                        Parent);
    }
}
//---------------------------------------------------------------------------
...
#define RE_CREATE(place, type, classname)                                      \
    ...
    RE_CREATE(NonLocalFunctionDeclVector, std::vector<tNonLocalFunctionDecl>,
              vector);
}
...
//---------------------------------------------------------------------------
ttstr tTJSInterCodeContext::GetPositionDescriptionString(
    tjs_int codepos) const {
...
tjs_int tTJSInterCodeContext::GenNodeCode(tjs_int &frame, tTJSExprNode *node,
    ...
    case T_IF: // 'if' operator
    {
        ...
        //		if(TJSIsModifySubType(param.SubType))
        //_yyerror(TJSCannotModifyLHS, Block);
        tjs_int resaddr = _GenNodeCode(
            frame, (*node)[1], TJS_RT_NEEDED | TJS_RT_CFLAG, 0, tSubParam());
        bool inv = false;
    ...
    case T_SWAP: // '<->' operator
    {
        ...
        tjs_int resaddr1 =
            _GenNodeCode(frame, (*node)[0], TJS_RT_NEEDED, 0, tSubParam());
    ...
    case T_QUESTION: // '?' ':' operator
    {
        ...
        tjs_int resaddr1, resaddr2;
        int frame1, frame2;
        resaddr = _GenNodeCode(frame, (*node)[0], TJS_RT_NEEDED | TJS_RT_CFLAG,
                               0, tSubParam());
        bool inv = false;
        if (!TJSIsCondFlagRetValue(resaddr)) {
    ...
    case T_EXCRAMATION: // pre-positioned '!' operator
    {
        ...
        if (TJSIsModifySubType(param.SubType))
            _yyerror(TJSCannotModifyLHS, Block);
        resaddr =
            _GenNodeCode(frame, (*node)[0], restype, reqresaddr, tSubParam());
        if (!(restype & TJS_RT_CFLAG)) {
    ...
    case T_NEW:          // 'new' operator
    {
        ...
        try {
            ...
            if (do_direct_access) {
                ...
                res = _GenNodeCode(frame, (*node)[0], restype, reqresaddr,
                                   param2);
            } else {
    ...
    case T_ARG:
        ...
        if ((*node)[0]) {
            ...
            if (n->GetOpecode() == T_EXPANDARG) {
                ...
                AddFuncArg(_GenNodeCode(frame, (*node)[0], TJS_RT_NEEDED, 0,
                                        tSubParam()),
                           fatNormal);
            }
        } else {
    ...
    case T_PROPACCESS: // unary '*' operator
        if (node->GetOpecode() ==
            ...
            case stSR:
                ...
                // ( see the ope-code's positioning order )
                PutCode(
                    TJS_TO_VM_REG_ADDR((restype & TJS_RT_NEEDED) ? frame : 0),
                    node_pos);
                PutCode(TJS_TO_VM_REG_ADDR(resaddr), node_pos);
                PutCode(TJS_TO_VM_REG_ADDR(param.SubAddress), node_pos);
                if (restype & TJS_RT_NEEDED)
            ...
            case stPreInc:
            case stPreDec:
                PutCode((param.SubType == stPreInc ? VM_INC : VM_DEC) + 3,
                        node_pos);
                PutCode(
                    TJS_TO_VM_REG_ADDR((restype & TJS_RT_NEEDED) ? frame : 0),
                    node_pos);
            ...
            case stPostDec: {
                ...
                PutCode((param.SubType == stPostInc ? VM_INC : VM_DEC) + 3,
                        node_pos);
                PutCode(TJS_TO_VM_REG_ADDR(0), node_pos);
                PutCode(TJS_TO_VM_REG_ADDR(resaddr), node_pos);
                return retresaddr;
    ...
    case T_ARRAYARG: {
        ...
        resaddr = (*node)[0] ? _GenNodeCode(frame, (*node)[0], TJS_RT_NEEDED, 0,
                                            tSubParam())
                             : 0;
...
tTJSInterCodeContext::ExportByteCode(bool outputdebug, tTJSScriptBlock *block,
    ...
    int size =
        12 * 4 + srcpossize + codesize + datasize + scgpsize + propsize + 4 * 4;
