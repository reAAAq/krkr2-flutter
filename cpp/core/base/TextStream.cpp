...
bool TVPStringEncode(const ttstr &src, std::string &result,
    ...
    if (encoding == enc_utf8 || encoding == enc_utf8_2) {
        ...
        // 	} else if (encoding == enc_jis || encoding == enc_jis_2 ||
        // encoding == enc_jis_3 || encoding == enc_jis_4) { 	} else if
        // (encoding == enc_gbk) {
        // // unsupported yet
    } else {
        return false;
...
//---------------------------------------------------------------------------
/*
        note: encryption of mode 0 or 1 ( simple crypt ) does never intend data
   pretection security.
*/
    ...
    tTVPTextReadStream(const ttstr &name, const ttstr &modestr) {
        ...
        try {
            ...
            if (mark[0] == 0xff && mark[1] == 0xfe) {
                ...
                // currently only mode0 and mode1, and compressed (mode2) are
                // supported
                CryptMode = mode;
                ...
                Stream->Read(
                    mark,
                    2); // original bom code comes here (is not compressed)
                if (mark[0] != 0xff || mark[1] != 0xfe)
                    TVPThrowExceptionMessage(TVPUnsupportedCipherMode, name);
