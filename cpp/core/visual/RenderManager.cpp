...
static const tjs_uint8 __empty_line[EMPTY_LINE_BYTES + 32] =
    {}; // at most 2048 pixels per line
...
public:
    virtual const void *GetPixelData() override {
        if (!BmpData) {
            ...
            tjs_int w = Width * (GetFormat() == TVPTextureFormat::RGBA
                                     ? sizeof(tjs_uint32)
                                     : sizeof(tjs_uint8));
...
class tTVPSoftwareTexture2D : public tTVPSoftwareTexture2D_static {
    tTVPSoftwareTexture2D(tTVPBitmap *bmp)
        : tTVPSoftwareTexture2D_static(bmp->GetBits(), bmp->GetPitch(),
...
public:
    virtual void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar,
        ...
        if (rcsrc.top > rcsrc.bottom) {
            ...
            // 			tjs_uint8 *src =
            // (tjs_uint8*)_src->GetScanLineForRead(rcsrc.top - 1) + rcsrc.left
            // * pixelsize; 			tjs_uint8 *dst =
            // (tjs_uint8*)_tar->GetScanLineForRead(rctar.top) + rctar.left *
            // pixelsize; 			tjs_int spitch = -_src->GetPitch(),
            // dpitch
            // = _tar->GetPitch(); 			tjs_int wbytes = w *
            // pixelsize; 			while (h--) { 				memcpy(dst,
            ...
            assert(rcsrc.get_width() == -w && rcsrc.get_height() == h);
            // 			tjs_uint8 *src =
            // (tjs_uint8*)_src->GetScanLineForRead(rcsrc.top) + rcsrc.right *
            // pixelsize; 			tjs_uint8 *dst =
            // (tjs_uint8*)_tar->GetScanLineForRead(rctar.top) + rctar.left *
            // pixelsize; 			tjs_int spitch = _src->GetPitch(),
            // dpitch = _tar->GetPitch();
            tjs_int wbytes = w * pixelsize;
            ...
            if (pixelsize == 4) { // 32bpp
                ...
                // 					memcpy(dst, src,
                // wbytes);
                // TVPReverse32((tjs_uint32*)dst, w); src += spitch; dst +=
                // dpitch;
                // 				}
            } else { // 8bpp
                for (int y = 0; y < h; ++y) {
            ...
            // rcsrc.left * pixelsize; 			} else {
            // src = (tjs_uint8*)_src->GetScanLineForRead(rcsrc.top) +
            // rcsrc.left * pixelsize; 				dest =
            // (tjs_uint8*)_tar->GetScanLineForWrite(rctar.top) + rctar.left *
            // pixelsize;
            // 			}
...
public:
    virtual void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar,
        ...
        tjs_int taskNum = GetAdaptiveThreadNum(w * h, THREAD_FACTOR);
        TVPExecThreadTask(taskNum, [this, _src, _tar, sx, sy, dx, dy, w, h,
                                    taskNum](int i) {
            tjs_int y0, y1;
            y0 = h * i / taskNum;
            y1 = h * (i + 1) / taskNum;
...
public:
    virtual void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar,
        ...
        // (tjs_uint8*)_dst->GetScanLineForRead(rcdst.top) + rcdst.left *
        // sizeof(tjs_uint32); 		tar =
        // (tjs_uint8*)_tar->GetScanLineForWrite(rctar.top) + rctar.left *
        // sizeof(tjs_uint32);
...
public:
    virtual void DoRender(iTVPTexture2D *_tar, const tTVPRect &rctar,
        ...
        // sizeof(tjs_uint32); 		tar =
        // (tjs_uint8*)_tar->GetScanLineForWrite(rctar.top) + rctar.left *
        // sizeof(tjs_uint32); 		rule =
        // (tjs_uint8*)_rule->GetScanLineForRead(rcrule.top) + rcrule.left *
        // sizeof(tjs_uint8);
    ...
    virtual void PartialProc(iTVPTexture2D *tar, tjs_int tx, tjs_int ty,
                             ...
                             tjs_int w, tjs_int h) {
        // 		tjs_uint8 *dst = p->dest, *src = p->src, *tar = p->tar,
        // *rule = p->rule; 		tjs_int width = p->w; 		tjs_int
        // height = p->h;
        ...
        if (vague >= 512) {
            ...
            // tjs_uint32*)src, (const tjs_uint32*)dst, rule, BlendTable,
            // width); 				src
            // += spitch; dst += dpitch; tar += tpitch; rule += rpitch;
            // 			}
        } else {
            ...
            // tjs_uint32*)src, (const tjs_uint32*)dst, rule, BlendTable, width,
            // src1lv, src2lv); 				src += spitch; dst += dpitch;
            // tar
            // += tpitch; rule += rpitch;
            // 			}
        }
...
// some blur operation template functions to select algorithm by base integer
// type
...
static void _log_ptr(const char *n, void *_p, uint l) {
    ...
    TVPConsoleLog(
        "%s %p: %02X %02X %02X %02X %02X %02X %02X %02X ... %p: %02X %02X %02X "
        "%02X ... %p: %02X %02X %02X %02X %02X %02X %02X %02X",
        n, p - 8, p[-8], p[-7], p[-6], p[-5], p[-4], p[-3], p[-2], p[-1], p,
        p[0], p[1], p[2], p[3], p + l, p[l + 0], p[l + 1], p[l + 2], p[l + 3],
        p[l + 4], p[l + 5], p[l + 6], p[l + 7], p[l + 8]);
}

template <typename tARGB> struct TDoBoxBlurLoop {
    ...
    static void DoBoxBlurLoop(const tTVPRect &rect, const tTVPRect &area,
        ...
        try {
            ...
            tjs_int h_init_start =
                rect.left + area.left; // this always be the same value as
                                       // vert_sum_left_limit
            if (h_init_start < 0)
                h_init_start = 0;
            ...
            if (dest_buf_size) {
                for (tjs_int i = 0; i < dest_buf_size; i++) {
                    //	_log_ptr("free  bufN", &dest_buf[i][0], (rect.right -
                    // rect.left)
                    //* 4);
...
public:
    ...
    virtual void DoRender(iTVPTexture2D *tar, const tTVPRect &rctar,
        ...
        // _tar->GetWidth(), _tar->GetHeight(), (tjs_uint32*)line, pitchBytes);
...
struct tRenderMethodCache {
    ...
    tRenderMethodCache(iTVPRenderManager *mgr) {
        ...
        Info[bmAddAlphaOnAddAlpha].Init(mgr, nullptr, nullptr,
                                        ...
                                        "AdditiveAlphaBlend_a");
        // Info[bmAddAlphaOnAlpha].Init(mgr, nullptr, nullptr, nullptr,
        // nullptr);
...
#define REGISER_BLEND_WITH_HDA_4(THREAD_FACTOR, NAME)                          \
    ...
    tTVPSoftwareRenderManager()
        : StretchType(stNearest), tempTexture(nullptr),
          img_convert_ctx(nullptr), _drawCount(0) {
    ...
    virtual void SetParameterInt(int id, int Value) {
        ...
        case eParameters::StretchType:
            ...
            if (StretchType > sizeof(cvFlags) / sizeof(cvFlags[0])) {
                StretchType =
                    (tTVPBBStretchType)(sizeof(cvFlags) / sizeof(cvFlags[0]) -
                                        1);
            }
            break;
        default:
...
// #endif
    ...
    virtual void OperateRect(iTVPRenderMethod *method, iTVPTexture2D *tar,
        ...
        case 0: // fill tar
            ((tTVPRenderMethod_Software *)method)
                ->DoRender(tar, rctar, nullptr, rctar, reftar, rctar, nullptr,
                           rctar);
            break;
        case 1: // src -> tar
        ...
        case 2: // src x dst -> tar
            ((tTVPRenderMethod_Software *)method)
                ->DoRender(tar, rctar, textures[1].first, textures[1].second,
                           textures[0].first, textures[0].second, nullptr,
                           rctar);
            break;
...
#define TVP_BILINEAR_FORCE_COND false
            _stretchMethodFactory[GetRenderMethod("Copy")] =
                ...
                if (TVP_BILINEAR_FORCE_COND ||
                    ...
                    // 				} else { // !hda
                    // 					affineloop = [](tjs_int sxs, tjs_int
                    // sys, tjs_uint8 *dest, tjs_int l, tjs_int len, const
                    // tjs_uint8 *src, tjs_int srcpitch, tjs_int sxl, tjs_int
                    // syl, 						const tTVPRect
                    // & srccliprect, const tTVPRect & srcrect){
                    // TVPDoAffineLoop(
    ...
    OperateTriangles(iTVPRenderMethod *method, int nTriangles,
        ...
        if (nTriangles == 2 && CheckQuad(pttar) && CheckTextureQuad(textures)) {
            ...
            while (isSrcRect && isDstRect) {
                ...
                // memset((tjs_uint8*)target->GetScanLineForWrite(i) + cr.left,
                // 0, clipW * 4);
                // 					}
                // 				}
            ...
            if (isSrcRect && checkQuadSquared(dstpt)) {
                ...
                // %d;%d, %d) ->
                // (%d, %d;%d, %d;%d, %d;%d, %d)\n",
                // (int)pts_src[0].x, (int)pts_src[0].y, (int)pts_src[1].x,
                // (int)pts_src[1].y,
                // (int)pts_src[2].x, (int)pts_src[2].y, (int)pts_src[3].x,
                // (int)pts_src[3].y, (int)pts_dst[0].x, (int)pts_dst[0].y,
                // (int)pts_dst[1].x, (int)pts_dst[1].y,
                // (int)pts_dst[2].x, (int)pts_dst[2].y, (int)pts_dst[3].x,
                // (int)pts_dst[3].y
                // 					);
                // #endif
            ...
            // dst->GetWidth(); 			if (rc.bottom > dst->GetHeight())
            // rc.bottom = dst->GetHeight();
            ...
            // TVPThrowExceptionMessage(TJS_W("OperateTriangles: unsupported
...
#define FIX_SX_SY                                                              \
    ...
    int InternalAffineBlt(/*CopyBitmapParam *param,*/ tTVPRect destrect,
        ...
        // check each vertex and find most-top/most-bottom/most-left/most-right
        // points
        ...
        if (mode & stRefNoClip)
            ...
            srccliprect =
                srcrect; // clip; the source is limited to the source rectangle
        ...
        for (; yc <= yclim; yc += 65536, dest += destpitch) {
            ...
            if (rot) {
                ...
                case 0:
                    ll =
                        mul_16(points_x[1] - points_x[0], where0) + points_x[0];
                ...
                case 1:
                    ll =
                        mul_16(points_x[2] - points_x[1], where0) + points_x[1];
                ...
                case 2:
                    ll =
                        mul_16(points_x[0] - points_x[2], where0) + points_x[2];
                    sxl = refrect.left;
                ...
                case 0:
                    rr =
                        mul_16(points_x[1] - points_x[0], where1) + points_x[0];
                ...
                case 1:
                    rr =
                        mul_16(points_x[2] - points_x[1], where1) + points_x[1];
                ...
                case 2:
                    rr =
                        mul_16(points_x[0] - points_x[2], where1) + points_x[2];
                    sxr = refrect.left;
            ...
            // 0x8000000 were choosed to avoid special divition behavior around
            // zero
            ...
            if (len > 0) {
                ...
                // 0) (hda ? TVPFillColor : TVPFillARGB)((tjs_uint32*)dest +
                // destrect.left, clen, clearcolor);
                // int rr = r - 1; 					if (rr <
                // destrect.left) rr =
                // destrect.left; 					clen =
                // destrect.right
                // - rr; 					if (clen > 0)
                // (hda ? TVPFillColor : TVPFillARGB)((tjs_uint32*)dest + rr,
                // clen, clearcolor);
                // 				}
