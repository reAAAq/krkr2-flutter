...
static void TJSUninitStringHeap() {
    ...
    if (!TJSStringHeapList->empty()) {
        std::vector<tTJSVariantString *>::iterator c;
        for (c = TJSStringHeapList->end() - 1; c >= TJSStringHeapList->begin();
             c--) {
            tTJSVariantString *h = *c;
            for (tjs_int i = 0; i < HEAP_CAPACITY_INC; i++) {
                if (h[i].HeapFlag & HEAP_FLAG_USING) {
    ...
    type_char:
        ...
        case TJS_W('s'): {
            ...
            if (str) {
                ...
                if (s + width > allocsize) {
                    try {
                        tjs_uint inc_size;
                        ret->AppendBuffer(inc_size = s + width - allocsize +
        ...
        case TJS_W('X'): {
            ...
            if (!width_ind && !prec_ind) {
                ...
                tTVInteger integer = (params[in++])->AsInteger();
                // FIXME:
                buf = {fmt::sprintf(ttstr{fmt}.AsNarrowStdString(), ind[0],
                                    integer)};
            } else {
                if (in >= numparams)
                    TJS_eTJSVariantError(TJSBadParamCount);
        ...
        case TJS_W('G'): {
            ...
            if (!width_ind && !prec_ind) {
                ...
                // FIXME:
                buf = {
                    fmt::sprintf(ttstr{fmt}.AsNarrowStdString(), ind[0], real)};
            } else {
                if (in >= numparams)
                    TJS_eTJSVariantError(TJSBadParamCount);
                ind[0] = static_cast<int>((params[in++])->AsInteger());
