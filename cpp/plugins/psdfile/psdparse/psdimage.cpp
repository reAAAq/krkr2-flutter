...
template <>
inline uint32_t rgbaCompoToRgba32<uint32_t>(const ColorFormat &fmt, uint32_t _r,
                                            uint32_t _g, uint32_t _b,
                                            uint32_t _a) {
    pun32 r, g, b, a;
#ifdef BOOST_LITTLE_ENDIAN
    r.i = byteSwap32(_r);
...
//   32bit コンポーネントは浮動小数点なので特殊化で対応
template <>
inline uint32_t rgbaCompoToRgba32<uint32_t>(const ColorFormat &fmt, uint32_t r,
...
//   32bit コンポーネントは浮動小数点なので特殊化で対応
template <>
inline uint32_t cmykCompoToRgba32<uint32_t>(const ColorFormat &fmt, uint32_t _c,
                                            uint32_t _m, uint32_t _y,
...
//   32bit コンポーネントは浮動小数点なので特殊化で対応
template <>
inline uint32_t cmykCompoToRgba32<uint32_t>(const ColorFormat &fmt, uint32_t c,
...
void mergeChannelsCmyk(void *merged, int width, int height,
    ...
    if (chA) {
        ...
        for (int y = 0; y < height; y++) {
            ...
            for (int x = 0; x < linePixel; x++) {
                *pix++ =
                    cmykCompoToRgba32<T>(format, *cp++, *mp++, *yp++, *kp++);
            }
        }
    }
}
...
bool PSDFile::getLayerImage(LayerInfo &layer, void *buf,
...
#ifdef USE_ZLIB
            decodeZipWithoutPrediction(decodedChannel, bufSize, tmpSourceBuffer,
                                       dataLength);
#else
...
#ifdef USE_ZLIB
            decodeZipWithPrediction(decodedChannel, bufSize, tmpSourceBuffer,
                                    dataLength, width, height, header.depth);
#else
            memset(decodedChannel, 0xff, bufSize);
#endif
            break;
        ...
        if (maskChannelIndex >= 0 && alphaChannelIndex >= 0) {
            ...
            case 8:
                mergeMaskToAlpha<uint8_t>(
                    alphaChannel, layer.left, layer.top, layer.right,
                    layer.bottom, maskChannel, mask.left, mask.top, mask.right,
                    mask.bottom, mask.defaultColor);
                break;
            case 16:
    ...
    case COLOR_MODE_BITMAP:
        mergeChannelsBitmap(buf, imageWidth, imageHeight, decodedChannels[0],
                            format, bufPitchByte);
        break;
    case COLOR_MODE_GRAYSCALE:
        ...
        case 8:
            ...
            break;
        case 16:
            mergeChannelsGray<uint16_t>(buf, imageWidth, imageHeight,
                                        channelIds, decodedChannels, format,
                                        bufPitchByte);
            break;
        case 32:
            mergeChannelsGray<uint32_t>(buf, imageWidth, imageHeight,
                                        channelIds, decodedChannels, format,
                                        bufPitchByte);
            break;
        default:
    ...
    case COLOR_MODE_INDEXED:
        mergeChannelsIndex(buf, imageWidth, imageHeight, decodedChannels[0],
                           colorTable, format, bufPitchByte);
        break;
    case COLOR_MODE_CMYK:
        ...
        case 8:
            ...
            break;
        case 16:
            mergeChannelsCmyk<uint16_t>(buf, imageWidth, imageHeight,
                                        channelIds, decodedChannels, format,
                                        bufPitchByte);
            break;
        case 32:
            mergeChannelsCmyk<uint32_t>(buf, imageWidth, imageHeight,
                                        channelIds, decodedChannels, format,
                                        bufPitchByte);
            break;
        default:
    ...
    case 3: // zip (w/ prediction)
    {
        for (int i = 0; i < channels; i++) {
            ...
            decodeZipWithPrediction(decodedChannels[i], imageChannelBytes,
                                    tmpSourceBuffer, dataLength, imageWidth,
                                    imageHeight, header.depth);
#else
            memset(decodedChannel, 0xff, bufSize);
#endif
        }
