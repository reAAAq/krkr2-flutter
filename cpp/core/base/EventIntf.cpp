...
void TVPDeliverAllEvents() {
    ...
    if (!TVPExclusiveEventPosted && !TVPEventInterrupting) {
        ...
        if (TVPProcessContinuousHandlerEventFlag) {
            ...
            // TVPProcessContinuousHandlerEventFlag may be accessed by another
            // thread. But I have no dought about that no one does care of
            // missing one event in rare race condition.
