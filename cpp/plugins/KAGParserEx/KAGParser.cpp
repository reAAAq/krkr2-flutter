...
void tTVPScenarioCacheItem::LoadScenario(const ttstr &name, bool isstring) {
    ...
    if(count == 0)
        TVPThrowExceptionMessage(TVP_KAGPARSER_MESSAGEMAP(TVPKAGNoLine), name);
...
//---------------------------------------------------------------------------
static tTVPScenarioCacheItem *TVPGetScenario(const ttstr &storagename,
                                             bool isstring) {
...

    {
        ...
        if(MacroArgStackDepth != MacroArgs.size())
            TVPThrowExceptionMessage(
                TVP_KAGPARSER_MESSAGEMAP(TVPKAGMalformedSaveData));
...
void tTJSNI_KAGParser::GoToLabel(const ttstr &name) {
    ...
    if(newline) {
        ...
        // not found
        TVPThrowExceptionMessage(TVP_KAGPARSER_MESSAGEMAP(TVPKAGLabelNotFound),
                                 StorageName, name);
    }
...
bool tTJSNI_KAGParser::SkipCommentOrLabel() {
    ...
    for(; CurLine < LineCount; CurLine++) {
        ...
        if(p[0] == TJS_W('*')) {
            ...
            if(RecordingMacro)
                TVPThrowExceptionMessage(
                    TVP_KAGPARSER_MESSAGEMAP(TVPLabelOrScriptInMacro));
        ...
        if(p[0] == TJS_W('[') &&
           ...
           p[0] == TJS_W('@') && (!TJS_strcmp(p, TJS_W("@iscript")))) {
            ...
            if(CurLine == LineCount)
                TVPThrowExceptionMessage(
                    TVP_KAGPARSER_MESSAGEMAP(TVPKAGInlineScriptNotEnd));
...
tjs_error tTJSNI_KAGParser::ArgValue::getProp(ttstr &name,
...
void tTJSNI_KAGParser::PopCallStack(const ttstr &storage, const ttstr &label) {
    ...
    if(CallStack.size() == 0)
        TVPThrowExceptionMessage(
            TVP_KAGPARSER_MESSAGEMAP(TVPKAGCallStackUnderflow));
    ...
    if(!storage.IsEmpty() || !label.IsEmpty()) {
        ...
        if(CurLine > LineCount)
            TVPThrowExceptionMessage(
                TVP_KAGPARSER_MESSAGEMAP(TVPKAGReturnLostSync));
...
bool tTJSNI_KAGParser::EntryParam(bool &condition, tTJSVariant &ValueVariant,
...
iTJSDispatch2 *tTJSNC_KAGParser::CreateNativeClass() {
    tTJSNativeClassForPlugin *classobj = TJSCreateNativeClassForPlugin(
        TJS_W("KAGParser"), &tTJSNC_KAGParser::CreateNativeInstance);
#else
    ...
    TJS_BEGIN_NATIVE_METHOD_DECL(/*func. name*/ assign) {
        ...
        if(clo.Object) {
            if(TJS_FAILED(clo.Object->NativeInstanceSupport(
                ...
                TVPThrowExceptionMessage(
                    TVP_KAGPARSER_MESSAGEMAP(TVPKAGSpecifyKAGParser));
        } else {
            TVPThrowExceptionMessage(
                TVP_KAGPARSER_MESSAGEMAP(TVPKAGSpecifyKAGParser));
        }
