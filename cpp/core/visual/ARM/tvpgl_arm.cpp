...
void do_interp_stretch_blend(CFunc c_func, OPFunc op_func, tjs_uint32 *dest,
    ...
    while (dest < pVecEndDst) {
        ...
        uint16x8_t s_b16 =
            vmulq_u16(vsubl_u8(a.val[0], b.val[0]), ratio); // qreg = 9
...
static uint8x8x4_t do_copy_src(uint8x8x4_t s, uint8x8x4_t d) { return s; }

#ifndef Region_AlphaBlend
static uint8x8x4_t do_AlphaBlend(uint8x8x4_t s, uint8x8x4_t d) {
    // d + s * a - d * a
    d.val[0] = vadd_u8(d.val[0], vsubhn_u16(vmull_u8(s.val[0], s.val[3]),
                                            vmull_u8(d.val[0], s.val[3])));
    d.val[1] = vadd_u8(d.val[1], vsubhn_u16(vmull_u8(s.val[1], s.val[3]),
                                            vmull_u8(d.val[1], s.val[3])));
    d.val[2] = vadd_u8(d.val[2], vsubhn_u16(vmull_u8(s.val[2], s.val[3]),
                                            vmull_u8(d.val[2], s.val[3])));
    return d;
}
...
static uint8x8x4_t do_ConstAlphaBlend_SD(uint8x8x4_t s, uint8x8x4_t d,
                                         tjs_int opa) {
    d.val[3] =
        vadd_u8(d.val[3], vsubhn_u16(vmull_u8(s.val[3], vdup_n_u8(opa)),
...
static uint8x8x4_t do_UnivTransBlend_a(uint8x8x4_t s2, uint8x8x4_t s1,
                                       uint8x8_t opa) {
    s1.val[3] = vadd_u8(s1.val[3], vsubhn_u16(vmull_u8(s2.val[3], opa),
                                              vmull_u8(s1.val[3], opa)));
...
static uint8x8x4_t do_PsOverlayBlend(uint8x8x4_t s, uint8x8x4_t d) {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        // threshold =
        // vand_u8(vmovn_u16(ms), threshold); 		s.val[i] =
        // vorr_u8(s.val[i], threshold);
    }
...
static tjs_uint32 *testdata2 = nullptr;
static tjs_uint32 *testdest1 = nullptr;
static tjs_uint32 *testdest2 = nullptr;
static tjs_uint32 *testtable = nullptr;
static tjs_uint8 *testrule = nullptr;
#include <stdlib.h>
#include <stdio.h>
...
#if !defined(WIN32) && 0
            const char bmphdr[] = "\x42\x4D\x36\x00\x04\x00\x00\x00\x00\x00\x36"
                                  "\x00\x00\x00\x28\x00\x00"
                                  "\x00\x00\x01\x00\x00\x00\x01\x00\x00\x01\x00"
                                  "\x20\x00\x00\x00\x00\x00"
                                  "\x00\x00\x00\x00\x12\x0B\x00\x00\x12\x0B\x00"
...
#define TEST_COUNT 200
        ...
        // 		REGISTER_TVPGL_CUSTOM_FUNC(TVPInterpStretchCopy,
        ...
        //         REGISTER_TVPGL_BLEND_FUNC_2(TVPColorDodgeBlend_HDA,
        //         TVPColorDodgeBlend); // performance issue
        //         REGISTER_TVPGL_ONLY(TVPColorDodgeBlend_HDA,
        //         TVPColorDodgeBlend_NEON);
        //         REGISTER_TVPGL_BLEND_FUNC(TVPColorDodgeBlend_HDA_o,
        //         TVPColorDodgeBlend_o, 100);
        //         REGISTER_TVPGL_ONLY(TVPColorDodgeBlend_HDA_o,
        //         TVPColorDodgeBlend_o_NEON);
