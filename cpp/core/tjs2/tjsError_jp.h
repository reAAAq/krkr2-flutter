...
TJS_MSG_DECL(TJSWarnEvalOperator,
             TJS_W("グローバルでない場所で後置 ! "
                   "演算子が使われています(この演算子の挙動はTJS2 version "
                   "2.4.1 で変わりましたのでご注意ください)"))
TJS_MSG_DECL(TJSNarrowToWideConversionError,
             TJS_W("ANSI 文字列を UNICODE "
                   "文字列に変換できません。現在のコードページで解釈できない文"
                   "字が含まれてます。正しいデータが指定されているかを確認して"
                   "ください。データが破損している可能性もあります"))
TJS_MSG_DECL(TJSVariantConvertError, TJS_W("%1 から %2 へ型を変換できません"))
TJS_MSG_DECL(
    TJSVariantConvertErrorToObject,
    TJS_W("%1 から Object へ型を変換できません。Object 型が要求される文脈で "
          "Object 型以外の値が渡されるとこのエラーが発生します"))
TJS_MSG_DECL(TJSIDExpected, TJS_W("識別子を指定してください"))
TJS_MSG_DECL(TJSSubstitutionInBooleanContext,
             TJS_W("論理値が求められている場所で = 演算子が使用されています(== "
                   "演算子の間違いですか？代入した上でゼロと値を比較したい場合"
                   "は、(A=B) != 0 の形式を使うことをお勧めします)"));
...
TJS_MSG_DECL(
    TJSSpecifyDicOrArray,
    TJS_W("Dictionary または Array クラスのオブジェクトを指定してください"))
TJS_MSG_DECL(TJSSpecifyArray,
...
TJS_MSG_DECL(TJSAccessDenyed, TJS_W("読み込み専用あるいは書き込み専用プロパティ"
                                    "に対して行えない操作をしようとしました"))
...
TJS_MSG_DECL(TJSCallHistoryIsFromOutOfTJS2Script,
             TJS_W("[TJSスクリプト管理外]"))
TJS_MSG_DECL(TJSNObjectsWasNotFreed,
...
TJS_MSG_DECL(TJSWarnRunningCodeOnDeletingObject,
             TJS_W("%4: 削除中のオブジェクト %1[%2] "
                   "上でコードが実行されています。このオブジェクトの作成時の呼"
                   "び出し履歴は以下の通りです:\r\n                     %3"))
#else
TJS_MSG_DECL(TJSWarnRunningCodeOnDeletingObject,
             TJS_W("%4: 削除中のオブジェクト %1[%2] "
...
TJS_MSG_DECL(TJSReadError,
             TJS_W("読み込みエラーが発生しました。ファイルが破損している可能性"
                   "や、デバイスからの読み込みに失敗した可能性があります"))
TJS_MSG_DECL(TJSSeekError,
             TJS_W("シークエラーが発生しました。ファイルが破損している可能性や"
                   "、デバイスからの読み込みに失敗した可能性があります"))

TJS_MSG_DECL(TJSByteCodeBroken,
             TJS_W("バイトコードファイル読み込みエラー。ファイルが壊れているか"
                   "バイトコードとは異なるファイルです"))
//---------------------------------------------------------------------------
