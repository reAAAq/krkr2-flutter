...
tTJSDictionaryObject *tTJSBinarySerializer::CreateDictionary(tjs_uint count) {
    ...
    DicClass->CreateNew(0, nullptr, nullptr, (iTJSDispatch2 **)&dic, 1, pparam,
                        DicClass);
    return dic;
}
...
void tTJSBinarySerializer::InsertArray(tTJSArrayObject *array, tjs_uint index,
                                       tTJSVariant *value) {
    if (value == nullptr)
        TJS_eTJSError(TJSReadError);
    tTJSArrayNI *ni = nullptr;
    tjs_error hr = array->NativeInstanceSupport(
