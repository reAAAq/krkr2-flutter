...
public:
    tTVPPreferenceInfoCheckBox(const std::string &cap, const std::string &key,
                               bool defval)
...
// { 	typedef tTVPPreferenceInfoSelectList inherit; public:
// 	tTVPPreferenceInfoSelectRenderer(const std::string &cap, const
// std::string &key, const std::string &defval, 		const
// std::initializer_list<std::pair<std::string, std::string> > &listinfo) :
// inherit(cap, key, defval, listinfo) {} 	virtual void onSetValue(const
// std::string &v) { 		inherit::onSetValue(v); 		if (v ==
// "opengl") { 			TVPOnOpenGLRendererSelected(true);
// 		}
// 	}
// };
...
public:
    tTVPPreferenceInfoSelectFile(const std::string &cap, const std::string &key,
                                 const std::string &defval)
        : tTVPPreferenceInfo<std::string>(cap, key, defval) {}
...
public:
    tTVPPreferenceInfoSliderIcon(const std::string &cap, const std::string &key,
                                 float defval)
...
public:
    tTVPPreferenceInfoSliderText(const std::string &cap, const std::string &key,
                                 float defval)
        : tTVPPreferenceInfo<float>(cap, key, defval) {}
    ...
    virtual iPreferenceItem *createItem(int idx) override {
        LocaleConfigManager *locmgr = LocaleConfigManager::GetInstance();
        tPreferenceItemTextSlider *ret =
            new tPreferenceItemTextSlider(DefaultValue, convertPercentScale);
        ret->autorelease();
        ret->_getter = std::bind(&PreferenceGetValueFloat, Key, DefaultValue);
        ret->_setter =
            std::bind(&PreferenceSetValueFloat, Key, std::placeholders::_1);
...
static void initAllConfig() {
    ...
    RootPreference.Preferences = {
        ...
        new tTVPPreferenceInfoSelectList(
            "preference_fps_limit", "fps_limit", "60",
            {{"60", "60"}, {"45", "45"}, {"30", "30"}, {"15", "15"}}),
        new tTVPPreferenceInfoSelectList("preference_select_renderer",
                                         "renderer", "software",
    ...
    SoftRendererOptPreference.Preferences = {
        new tTVPPreferenceInfoSelectList("preference_multi_draw_thread",
        ...
        new tTVPPreferenceInfoSelectList(
            "preference_software_compress_tex", "software_compress_tex", "none",
    ...
    OpenglOptPreference.Preferences = {
        new tTVPPreferenceInfoSubPref(
            "preference_opengl_extension_opt",
            {
                ...
                new tTVPPreferenceInfoCheckBox("GL_NV_shader_framebuffer_fetch",
                ...
                new tTVPPreferenceInfoCheckBox("GL_EXT_clear_texture",
                                               "GL_EXT_clear_texture", true),
                new tTVPPreferenceInfoCheckBox("GL_ARB_clear_texture",
