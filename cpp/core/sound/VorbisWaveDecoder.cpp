...
// static bool Look_Replay_Gain = false; // whether to look replay gain
// information static bool Use_Album_Gain = false; // whether to use album gain,
...
bool VorbisWaveDecoder::SetStream(const ttstr &url) {
        ...
        if (vc->vendor) {
            ...
            if (vorbis_date) {
                ...
                if (date < WARN_OLD_VORBIS_DATE) {
                    ...
                    TVPAddLog(
                        TJS_W("wuvorbis: warning: The Vorbis stream \"") +
                        ttstr(url) +
                        TJS_W("\" seems to be generated by old Vorbis encoder "
                              "[") +
                        ttstr(vc->vendor) +
                        TJS_W(
                            "]. Check that your encoder is new enough, unless "
                            "you prefer the old encoder. The old stream can "
    ...
    //             vorbis_comment_query(vc, "replaygain_album_gain", 0);
    //
    //             const char * sel = Use_Album_Gain ? ( album ? album : track )
    //             : track; if(sel)
    //             {
    //                 float db = (float)atof(sel); // in db
    //                 gain *= (float)pow(10.0, db / 20); // convert db to
    //                 multiplier
    //             }
    //
