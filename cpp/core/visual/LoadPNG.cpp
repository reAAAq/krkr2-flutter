     ...
     ((tjs_uint32)(a)[2] << 8) + ((tjs_uint32)(a)[3]))
        ...
        case PNG_OFFSET_PIXEL:
            user_struct->metainfopushcallback(user_struct->callbackdata,
                                              PNG_tag_vpag_unit, PNG_tag_pixel);
            break;
        ...
        default:
            user_struct->metainfopushcallback(
                user_struct->callbackdata, PNG_tag_vpag_unit, PNG_tag_unknown);
            break;
        }
...
void TVPLoadPNG(void *formatdata, void *callbackdata,
    ...
    try {
        ...
        int reso_unit_type;
        if (metainfopushcallback && png_get_pHYs(png_ptr, info_ptr, &reso_x,
                                                 &reso_y, &reso_unit_type)) {
            // push offset information into metainfo data
    ...
    try {
        ...
        *dic = TJSCreateDictionaryObject();
        ...
        if (png_get_oFFs(png_ptr, info_ptr, &offset_x, &offset_y,
            ...
            (*dic)->PropSet(TJS_MEMBERENSURE, TJS_W("offset x"), 0, &val,
                            (*dic));
            val = tTJSVariant(offset_y);
            (*dic)->PropSet(TJS_MEMBERENSURE, TJS_W("offset y"), 0, &val,
                            (*dic));
            switch (offset_unit_type) {
            case PNG_OFFSET_PIXEL:
            ...
            (*dic)->PropSet(TJS_MEMBERENSURE, TJS_W("offset unit"), 0, &val,
                            (*dic));
        }

        png_uint_32 reso_x, reso_y;
        int reso_unit_type;
        if (png_get_pHYs(png_ptr, info_ptr, &reso_x, &reso_y,
                         &reso_unit_type)) {
            val = tTJSVariant((tjs_int64)reso_x);
            (*dic)->PropSet(TJS_MEMBERENSURE, TJS_W("resolution x"), 0, &val,
                            (*dic));
            val = tTJSVariant((tjs_int64)reso_y);
            (*dic)->PropSet(TJS_MEMBERENSURE, TJS_W("resolution y"), 0, &val,
                            (*dic));
            switch (reso_unit_type) {
            case PNG_RESOLUTION_METER:
            ...
            (*dic)->PropSet(TJS_MEMBERENSURE, TJS_W("resolution unit"), 0, &val,
                            (*dic));
        }
    } catch (...) {
        png_destroy_read_struct(&png_ptr, &info_ptr, &end_info);
