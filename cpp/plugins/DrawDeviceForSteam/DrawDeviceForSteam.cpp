...
public:
    ...
    static tjs_error factory(DrawDeviceForSteam** result, tjs_int numparams, tTJSVariant** params, iTJSDispatch2* objthis){
    ...
    // 以下所有纯虚函数全部透传
    //--------------------------------------------------
    void Destruct() override { if (Real) Real->Destruct(); }
    ...
    {
        if (Real) Real->AddLayerManager(manager);
    }

    void RemoveLayerManager(iTVPLayerManager* manager) override
    {
        if (Real) Real->RemoveLayerManager(manager);
    }

    void SetDestRectangle(const tTVPRect& rect) override
    {
        if (Real) Real->SetDestRectangle(rect);
    }
    ...
    {
        if (Real) Real->SetClipRectangle(rect);
    }
    ...
    {
        if (Real) Real->NotifyLayerResize(manager);
    }

    void NotifyLayerImageChange(iTVPLayerManager* manager) override
    {
        if (Real) Real->NotifyLayerImageChange(manager);
    }
    ...
    {
        if (Real) Real->OnTouchScaling(startdist, curdist, cx, cy, flag);
    }

    void OnTouchRotate(tjs_real startangle, tjs_real curangle, tjs_real dist, tjs_real cx, tjs_real cy, tjs_int flag) override
    {
        if (Real) Real->OnTouchRotate(startangle, curangle, dist, cx, cy, flag);
    }
    ...
    {
        if (Real) Real->OnDisplayRotate(orientation, rotate, bpp, width, height);
    }
    ...
    {
        if (Real) Real->SetFocusedLayer(layer);
    }

    void RequestInvalidation(const tTVPRect& rect) override
    {
        if (Real) Real->RequestInvalidation(rect);
    }
    ...
    {
        if (Real) Real->StartBitmapCompletion(manager);
    }
    ...
    {
        if (Real) Real->NotifyBitmapCompleted(manager, x, y, bmp, cliprect, type, opacity);
    }

    void EndBitmapCompletion(iTVPLayerManager* manager) override
    {
        if (Real) Real->EndBitmapCompletion(manager);
    }
    ...
    {
        return Real ? Real->SwitchToFullScreen(window, w, h, bpp, color, changeresolution) : false;
    }

    void RevertFromFullScreen(int window, tjs_uint w, tjs_uint h,
                              tjs_uint bpp, tjs_uint color) override
    {
        if (Real) Real->RevertFromFullScreen(window, w, h, bpp, color);
    }
...
// 2. 工厂函数：new 一个壳并返回
//----------------------------------
tjs_error  CreateDrawDeviceForSteam(
    iTJSDispatch2* objthis,          // 会被 simplebinder 传进来
...
// 3. 析构函数：析构时恢复原设备（可选）
//----------------------------------
tjs_error  DestroyDrawDeviceForSteam(
    iTJSDispatch2* objthis,          // 会被 simplebinder 传进来
...
public:
    ...
    {
        if (CppInstance) delete CppInstance; // 如果之前已经创建，先清理 (不太可能在这里发生)
        CppInstance = new DrawDeviceForSteam(); // 创建 C++ 对象
        TVPMainWindow->DrawDevice = CppInstance; // 如果你还想替换全局的

        // 将 C++ 实例与 tTJSNativeInstance 关联 (对于 ncbind，这通常在 CreateNativeInstance 中完成)
        // SetNativeInstance(CppInstance); // 假设 ncbind 提供了这样的机制或 tTJSNI_Skel 做了这个
        return TJS_S_OK;
    }
    void Invalidate() override {
        ...
        if (CppInstance) {
            ...
            // 你可能需要在这里恢复它或者采取其他措施。
            if (TVPMainWindow->DrawDevice == CppInstance) TVPMainWindow->DrawDevice = CppInstance->GetReal();
            delete CppInstance;
            CppInstance = nullptr;
        }
    }
