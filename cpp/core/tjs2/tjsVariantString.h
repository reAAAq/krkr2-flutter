...
public:
    ...
    TJS_METHOD_DEF(void, SetString,
                   (const tjs_char *ref, tjs_int maxlen = -1)) {
        if (LongString)
    ...
    TJS_METHOD_DEF(void, SetString, (const tjs_nchar *ref)) {
        ...
        if (len > TJS_VS_SHORT_LEN) {
            ...
            ShortString[TJS_narrowtowide(ShortString, ref, TJS_VS_SHORT_LEN)] =
                0;
        }
    }
    ...
    TJS_CONST_METHOD_DEF(tjs_int, GetRefCount, ()) { return RefCount; }
