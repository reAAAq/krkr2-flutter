...
tTVPLayerTreeOwner::RegisterLayerManager(class iTVPLayerManager *manager) {
    // Managers に manager を push する。AddRefするのを忘れないこと。
    Managers.push_back(manager);
    manager->AddRef();
}
void TJS_INTF_METHOD
tTVPLayerTreeOwner::UnregisterLayerManager(class iTVPLayerManager *manager) {
...
void TJS_INTF_METHOD tTVPLayerTreeOwner::SetMouseCursor(
    class iTVPLayerManager *manager, tjs_int cursor) {
...
void TJS_INTF_METHOD tTVPLayerTreeOwner::GetCursorPos(
...
void TJS_INTF_METHOD tTVPLayerTreeOwner::SetCursorPos(
    class iTVPLayerManager *manager, tjs_int x, tjs_int y) {
    iTVPLayerManager *primary_manager =
        GetLayerManagerAt(PrimaryLayerManagerIndex);
    if (!primary_manager)
...
void TJS_INTF_METHOD
tTVPLayerTreeOwner::ReleaseMouseCapture(class iTVPLayerManager *manager) {
    iTVPLayerManager *primary_manager =
        GetLayerManagerAt(PrimaryLayerManagerIndex);
    if (!primary_manager)
        return;
...
void TJS_INTF_METHOD tTVPLayerTreeOwner::SetHint(
    class iTVPLayerManager *manager, iTJSDispatch2 *sender, const ttstr &hint) {
    iTVPLayerManager *primary_manager =
        GetLayerManagerAt(PrimaryLayerManagerIndex);
    if (!primary_manager)
        return;
...
void TJS_INTF_METHOD
tTVPLayerTreeOwner::NotifyLayerResize(class iTVPLayerManager *manager) {
    iTVPLayerManager *primary_manager =
        GetLayerManagerAt(PrimaryLayerManagerIndex);
    if (primary_manager == manager) {
        tjs_int w, h;
...
void TJS_INTF_METHOD
tTVPLayerTreeOwner::NotifyLayerImageChange(class iTVPLayerManager *manager) {
    // change layer image
...
void TJS_INTF_METHOD tTVPLayerTreeOwner::SetAttentionPoint(
    class iTVPLayerManager *manager, tTJSNI_BaseLayer *layer, tjs_int x,
    tjs_int y) {
    iTVPLayerManager *primary_manager =
        GetLayerManagerAt(PrimaryLayerManagerIndex);
    if (!primary_manager)
...
void TJS_INTF_METHOD
tTVPLayerTreeOwner::DisableAttentionPoint(class iTVPLayerManager *manager) {
    iTVPLayerManager *primary_manager =
        GetLayerManagerAt(PrimaryLayerManagerIndex);
    if (!primary_manager)
        return;
...
void TJS_INTF_METHOD
tTVPLayerTreeOwner::SetImeMode(class iTVPLayerManager *manager, tjs_int mode) {
    iTVPLayerManager *primary_manager =
        GetLayerManagerAt(PrimaryLayerManagerIndex);
    if (!primary_manager)
        return;
...
void TJS_INTF_METHOD
tTVPLayerTreeOwner::ResetImeMode(class iTVPLayerManager *manager) {
    iTVPLayerManager *primary_manager =
        GetLayerManagerAt(PrimaryLayerManagerIndex);
    if (!primary_manager)
        return;
