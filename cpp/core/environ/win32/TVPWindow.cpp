...
LRESULT WINAPI tTVPWindow::Proc(HWND hWnd, UINT msg, WPARAM wParam,
    ...
    case WM_XBUTTONDOWN: // 戻るや進むが割り当てられる
        switch (GET_XBUTTON_WPARAM(wParam)) {
        case XBUTTON1: // サイドキーが第1Xボタン
            OnMouseDown(mbX1, GetShiftState(wParam), GET_X_LPARAM(lParam),
                        GET_Y_LPARAM(lParam));
            return 0;
        case XBUTTON2: // サイドキーが第2Xボタン
            OnMouseDown(mbX2, GetShiftState(wParam), GET_X_LPARAM(lParam),
                        GET_Y_LPARAM(lParam));
            return 0;
        }
        return ::DefWindowProc(hWnd, msg, wParam, lParam);
                    ...
                    if (procGetTouchInputInfo((HTOUCHINPUT)lParam, cInputs,
                        ...
                        for (UINT i = 0; i < cInputs; i++) {
                            ...
                            if (pInputs[i].dwMask &
                                ...
                                cx = static_cast<double>(pInputs[i].cxContact) /
                                     100.0;
                                cy = static_cast<double>(pInputs[i].cyContact) /
                                     100.0;
                            }
                            DWORD tick;
                            if (pInputs[i].dwMask &
