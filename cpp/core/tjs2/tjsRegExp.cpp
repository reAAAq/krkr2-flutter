...
//---------------------------------------------------------------------------
void replace_regex(tTJSVariant **param, tjs_int numparams, tTJSNI_RegExp *_this,
                   iTJSDispatch2 *objthis, ttstr &res) {
    ttstr to;
    ...
    if (r >= 0) { // match
        do {
            ...
            if (!func) {
                ...
                array->Release();
                hr = funcval.FuncCall(0, nullptr, nullptr, &result, 1, &param,
                                      nullptr);
                if (TJS_FAILED(hr)) {
