...
public:
    // in order not to cause a performance hit, we should only check the clock
    // when we reach m_nEstimatedBitrate bits. if this value is 1, we will
    // calculate bitrate on every sample.
