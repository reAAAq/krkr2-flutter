...
typedef void(GLAPIENTRY fClearTexImage)(uint texture, int level, GLenum format,
                                        GLenum type, const void *data);
static fClearTexImage *glClearTexImage;
typedef void(GLAPIENTRY fClearTexSubImage)(uint texture, int level, int xoffset,
...
// (forceNotice ||
// GlobalConfigManager::GetInstance()->GetValue<int>("noticed_adreno_issue", 0)
// < 1) { 			const char *btnText[2] = {
...
protected:
    ...
    GetAdapterTexture(cocos2d::Texture2D *orig) override {
        if (orig) {
    ...
    virtual tjs_uint32 GetPoint(int x, int y) override {
        if (Bitmap->Is32bit()) {
            ...
            TVPThrowExceptionMessage(TJS_W(
                "Unsupported texture format[RGB] for getting point color."));
            return 0;
        }
    }
...
public:
    ...
    virtual void SetSize(unsigned int w, unsigned int h) { assert(false); }
...
protected:
    ...
    // 	typedef bool(tTVPOGLRenderMethod::*FCustomProc)(tTVPOGLTexture2D *,
...
public:
    ...
    virtual tTVPOGLRenderMethod *SetBlendFuncSeparate(int func, int srcRGB,
...
public:
    ...
    virtual int EnumParameterID(const char *name) { return 0; }
    virtual void SetParameterColor4B(int id, unsigned int clr) {
        if (CustomProc)
            m_color = clr;
        else
    ...
    static bool DoFill(tTVPOGLRenderMethod *method, tTVPOGLTexture2D *tar,
        ...
        if (check) {
            cv::Mat _src(tar->GetInternalHeight(), tar->GetInternalWidth(),
                         CV_8UC4);
            GL::glGetTextureImage(tar->texture, 0, GL_BGRA, GL_UNSIGNED_BYTE,
...
protected:
    ...
    static bool glew_dynamic_binding() {
        ...
        // If the current opengl driver doesn't have framebuffers methods, check
    ...
    case x:                                                                    \
        ...
        break;
            ...
            default: {
                ...
                TVPConsoleLog(
                    (std::string("glCheckFramebufferStatus = ") + tmp).c_str());
            } break;
            }
            if (reason) {
                TVPConsoleLog(
...
public:
    TVPRenderManager_OpenGL() : tempTexture(nullptr), _drawCount(0) {
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_AdjustGamma>(
            ...
            "{\n"
            "    vec4 s = texture2D(tex0, v_texCoord0);\n"
            "    s.rgb = clamp(pow(s.rgb, u_gamma) * u_amp + u_floor, 0.0, "
            "1.0);\n"
            "    gl_FragColor = s;\n"
            "}",
            1)
        ...
        std::string shader_AdjustGamma_a_2 =
            "    vec3 t = (s.rgb / (s.a + 0.001) - 1.0) * step(s.rgb, s.aaa + "
            "0.001) + 1.0;\n"
...
#define TEST_SHADER_BLTO(x)                                                    \
    TEST_SHADER(                                                               \
        ...
        if (GL::glAlphaFunc) {
            ...
            CompileAndRegScript<tTVPOGLRenderMethod_AlphaTest>(
                "AlphaTest",
                ScriptCommonPrefix + "    gl_FragColor = s;\n"
                                     "}",
                1)
                ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_ZERO, GL_ONE, GL_ZERO,
                                       GL_ONE);
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script_BlendColor>(
            "AlphaBlend_SD",
            /*opacityPrefix +*/ ScriptCommonPrefix + "    gl_FragColor = s;\n"
                                                     "}",
            1)
            ->SetBlendFuncSeparate(
                GL_FUNC_ADD, GL_CONSTANT_COLOR, GL_ONE_MINUS_CONSTANT_COLOR,
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_Script>(
            ...
            ->SetBlendFuncSeparate(GL_FUNC_ADD, GL_SRC_ALPHA,
                                   GL_ONE_MINUS_SRC_ALPHA, GL_ONE,
                                   GL_ONE_MINUS_SRC_ALPHA);
        TEST_SHADER(AlphaBlend_a, TVPAlphaBlend_ao(testdest, testdata1,
                                                   256 * 256, TEST_SHADER_OPA));
        ...
        CompileAndRegRegularBlendMethod("AlphaBlend_d", opacityPrefix,
                                        shader_AlphaBlend_d);
        TEST_SHADER(AlphaBlend_d, TVPAlphaBlend_do(testdest, testdata1,
                                                   256 * 256, TEST_SHADER_OPA));
        ...
        if (GL::glAlphaFunc) {
            ...
            CompileAndRegRegularBlendMethod(
                ...
                "    s *= color;\n"
                "    if(s.a < alpha_threshold) discard;\n"
                "    d.rgb = mix(d.rgb, clamp(d.rgb + s.rgb - 1.0, 0.0, 1.0), "
                "s.a);\n"
                "    gl_FragColor = d;\n"
                "}");
        }
        ...
        CompileAndRegRegularBlendMethod(
            ...
            "    vec3 sd = s.rgb * d.rgb;\n"
            "    s.rgb = step(0.5, d.rgb) * ((s.rgb + d.rgb) * 2.0 - sd * 4.0 "
            "- "
            "1.0) + sd * 2.0;\n"
        ...
        CompileAndRegRegularBlendMethod(
            ...
            "    vec3 sd = s.rgb * d.rgb;\n"
            "    s.rgb = step(0.5, s.rgb) * ((s.rgb + d.rgb) * 2.0 - sd * 4.0 "
            "- "
            "1.0) + sd * 2.0;\n"
            "    d.rgb = mix(d.rgb, s.rgb, s.a * opacity);\n"
            "    gl_FragColor = d;\n"
        ...
        CompileAndRegRegularBlendMethod(
            ...
            "    s.rgb = pow(d.rgb, (1.0 - step(0.5, s.rgb)) * ((1.0 - s.rgb) "
            "* "
            "2.0 - t) + t);\n"
            "    d.rgb = mix(d.rgb, s.rgb, s.a * opacity);\n"
            "    gl_FragColor = d;\n"
        ...
        CompileAndRegScript<tTVPOGLRenderMethod_BoxBlur>(
            ...
            "}",
            1) /*->SetTargetAsSrc()*/;
        // TEST_SHADER(BoxBlur, TVPDoGrayScale(testdest, 256 * 256)); let it
        // pass
#if 0
    ...
    tTVPOGLTexture2D *GetTempTexture2D(tTVPOGLTexture2D *src,
        ...
        if (!tempTexture || tempTexture->internalW < w ||
            ...
            // tempTexture = (tTVPOGLTexture2D *)CreateTexture2D(nullptr, 0, w,
            // h, TVPTextureFormat::RGBA);
        }
        tempTexture->Width = w;
    ...
    virtual iTVPTexture2D *CreateTexture2D(const void *pixel, int pitch,
                                           unsigned int w, unsigned int h,
                                           TVPTextureFormat::e format,
                                           int flags) override {
        if (format > TVPTextureFormat::Compressed) {
            ...
            default:
                TVPThrowExceptionMessage(
                    TJS_W("Unsupported compressed texture format [%1]"),
    ...
    void CopyTexture(tTVPOGLTexture2D *dst, tTVPOGLTexture2D *src,
        ...
        if (GL::glCopyImageSubData && !src->IsCompressed &&
            ...
            if (check) {
                ...
                GL::glGetTextureImage(
                    ...
                    src->internalH * src->internalW * 4, _src.ptr(0, 0));
                GL::glGetTextureImage(
                    dst->texture, 0, GL_BGRA, GL_UNSIGNED_BYTE,
                    dst->internalH * dst->internalW * 4, _dst.ptr(0, 0));
                dst = dst;
            }
#endif
            return;
        ...
        if (check) {
            cv::Mat _src(src->internalH, src->internalW, CV_8UC4);
            cv::Mat _dst(dst->internalH, dst->internalW, CV_8UC4);
            GL::glGetTextureImage(src->texture, 0, GL_BGRA, GL_UNSIGNED_BYTE,
                                  src->internalH * src->internalW * 4,
                                  _src.ptr(0, 0));
            GL::glGetTextureImage(dst->texture, 0, GL_BGRA, GL_UNSIGNED_BYTE,
    ...
    virtual void OperateRect(iTVPRenderMethod *_method, iTVPTexture2D *_tar,
        ...
        // if (!method->CustomProc || !(method->*(method->CustomProc))(tar,
        // rctar, texlist)) { float tw = (float)tar->internalW, th =
        // (float)tar->internalH;
        ...
        if (check) {
            ...
            for (unsigned int i = 0; i < texlist.size(); ++i) {
                _src[i] = new cv::Mat(texlist[i].tex->internalH,
                                      texlist[i].tex->internalW, CV_8UC4);
                GL::glGetTextureImage(
                    texlist[i].tex->texture, 0, GL_BGRA, GL_UNSIGNED_BYTE,
                    texlist[i].tex->internalH * texlist[i].tex->internalW * 4,
                    _src[i]->ptr(0, 0));
            }
            cv::Mat _tar(tar->internalH, tar->internalW, CV_8UC4);
            cv::Mat _stencil(tar->internalH, tar->internalW, CV_8U);
            GL::glGetTextureImage(tar->texture, 0, GL_BGRA, GL_UNSIGNED_BYTE,
                                  tar->internalH * tar->internalW * 4,
                                  _tar.ptr(0, 0));
    ...
    virtual void OperateTriangles(iTVPRenderMethod *_method, int nTriangles,
        ...
        // glClearColor(oldClearColor[0], oldClearColor[1], oldClearColor[2],
        // oldClearColor[3]); 		} else { 			static
        // tTVPOGLRenderMethod* _method =
    ...
    public:
        ...
        virtual void ApplyTexture(unsigned int i, const GLVertexInfo &info) {
            info.tex->Bind(i);
            // glVertexAttribPointer(GetTexCoordAttr(i), 2, GL_FLOAT, GL_FALSE,
            // 0, &info.vtx.front());
        }
    ...
    virtual void
    OperatePerspective(iTVPRenderMethod *_method, int nQuads,
                       iTVPTexture2D *_tar, iTVPTexture2D *reftar,
                       const tTVPRect &rcclip,
