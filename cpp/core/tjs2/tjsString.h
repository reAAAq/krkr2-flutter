...
public:
    //-------------------------------------------------------- constructor --
    TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, ()) { Ptr = nullptr; }
    ...
    TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, (const tjs_char *str)) {
        Ptr = TJSAllocVariantString(str);
    }

    TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString, (const tjs_nchar *str)) {
        Ptr = TJSAllocVariantString(str);
    }
    ...
    TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString,
                   (const tTJSString &str,
                    int n)) // construct with first n chars of str
    {
    ...
    TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString,
                   (const tjs_char *str,
                    int n)) // same as above except for str's type
    {
    ...
    TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString,
                   (tjs_int64 n)); // from int64

#ifdef TJS_SUPPORT_VCL
    tTJSString(const AnsiString &str) {
        Ptr = TJSAllocVariantString(str.c_str());
    }
    tTJSString(const WideString &str) {
        Ptr = TJSAllocVariantString(str.c_bstr());
    }
#endif

    tTJSString(const std::basic_string<tjs_char> &str) {
        Ptr = TJSAllocVariantString(str.c_str());
    }

    tTJSString(const std::string &str) {
        Ptr = TJSAllocVariantString(str.c_str());
    }
    ...
    [[nodiscard]] std::wstring c_wstr() const {
        return boost::locale::conv::utf_to_utf<wchar_t>(AsNarrowStdString());
    }
    ...
    TJS_METHOD_DEF(tjs_char *, AllocBuffer, (tjs_uint len)) {
        /* you must call FixLen when you allocate larger buffer than actual
         * string length */
    ...
    TJS_METHOD_DEF(void, Replace,
                   (const tTJSString &from, const tTJSString &to,
                    bool forall = true));
    ...
    TJS_CONST_METHOD_DEF(void, AsLowerCase, (tTJSString & dest)) {
        dest = AsLowerCase();
    }

    TJS_CONST_METHOD_DEF(void, AsUpperCase, (tTJSString & dest)) {
        dest = AsUpperCase();
    }
    ...
    size_t printf(const char *format, const T &...args) {
        std::u16string utf16_string = boost::locale::conv::utf_to_utf<char16_t>(
    ...
    TJS_CONST_METHOD_DEF(void, EscapeC, (tTJSString & dest)) {
        dest = EscapeC();
    }

    TJS_CONST_METHOD_DEF(void, UnescapeC, (tTJSString & dest)) {
        dest = UnescapeC();
    }
    ...
    TJS_CONST_METHOD_DEF(void, ToNarrowStr,
                         (tjs_nchar * dest, tjs_int destmaxlen));

    //------------------------------------------------------------- others --
    TJS_CONST_METHOD_DEF(bool, IsEmpty, ()) { return Ptr == nullptr; }
...
public:
    TJS_METHOD_DEF(tjs_char *, Independ, ()) {
        ...
        // note that you must call FixLen after making modification of the
        // buffer if you shorten the string using this method's return value.
...
#ifdef __CODEGUARD__
        if (!Ptr)
            return 0; // tTJSVariantString::GetLength can return zero if 'this'
                      // is nullptr
#endif
    ...
    int IndexOf(tjs_char c, unsigned int pos = 0) const {
...
/*end-of-tTJSString*/
TJS_EXP_FUNC_DEF(tTJSString, operator+,
