                ...
                case EC_COMPLETE:
                    if (Status == ssPlay) {
                        if (Loop) {
                            Rewind();
                            FirePeriodEvent(
                                perLoop); // fire period event by loop rewind
                        } else {
                            ...
                            VideoOverlay->Stop();
                            SetStatusAsync(
                                ssStop); // All data has been rendered
                        }
                    }
                    break;
                case EC_UPDATE:
                    if (Mode == vomLayer && Status == ssPlay) {
                        ...
                        if ((!IsPrepare) && (SegLoopEndFrame > 0) &&
                            ...
                            SetFrame(SegLoopStartFrame > 0 ? SegLoopStartFrame
                                                           : 0);
                        ...
                        if (!IsPrepare) {
                            ...
                            if (EventFrame >= 0 && !IsEventPast &&
                                ...
                                EventFrame = -1;
                                FirePeriodEvent(
                                    perPeriod); // fire period event by
                                                // setPeriodEvent()
                            }
                        } else { // Prepare mode
                            FirePeriodEvent(
                                perPrepare); // fire period event by prepare()
                            Pause();
                            Rewind();
                            IsPrepare = false;
                        }
