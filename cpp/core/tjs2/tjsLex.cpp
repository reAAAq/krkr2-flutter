...
TJSInternalParseString(tTJSVariant &val, const tjs_char **ptr, tjs_char delim,
                       bool embexpmode) {
    // delim1 must be '\'' or '"'
    // delim2 must be '&' or '\0'
...
static bool TJSParseNonDecimalReal(tTJSVariant &val, const tjs_char **ptr,
    ...
    tjs_int numsignif =
        0; // significand bit count (including leading left-most '1') in "main"
    ...
    main &=
        ((TJS_UI64_VAL(1) << TJS_IEEE_D_SIGNIFICAND_BITS) - TJS_UI64_VAL(1));
...
tTJSLexicalAnalyzer::tTJSLexicalAnalyzer(tTJSScriptBlock *block,
                                         const tjs_char *script, bool exprmode,
                                         bool resneeded) {
...
re_match:
    ...
    PrevPos =
        (tjs_int)(Current - Script); // remember current position as "PrevPos"
    ...
    *d = 0;
    ...
    if (BareWord)
        ...
        retnum =
            TJSReservedWordHash->GetValueInteger(str.c_str(), str.GetHint());
                ...
                if (tjsEnableDicFuncQuickHack) //----- dicfunc quick-hack
                {
                    if (DicFunc) {
                        if (n == T_PERCENT) {
                            ...
                            RetValDeque.push_back(
                                tTokenPair(T_LPARENTHESIS, 0));
                            RetValDeque.push_back(
                                tTokenPair(T_RPARENTHESIS, 0));
                            n = -1;
                        }
                    }
                }
                ...
                case evsNextIsStringLiteral: {
                    ...
                    if (res == psrDelimiter) {
                        ...
                        if (!str.IsEmpty() || !data.NeedPlus)
                            RetValDeque.push_back(
                                tTokenPair(T_CONSTVAL, PutValue(v)));
