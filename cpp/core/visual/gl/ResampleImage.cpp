...
#define TVP_BLEND_4(                                                           \
    basename) /* blend for 4 types (normal, opacity, HDA, HDA opacity) */      \
    if (opa_ == 255) {                                                         \
        if (!hda_) {                                                           \
            copy_func = basename;                                              \
        } else {                                                               \
 ...
 */
template <typename TWeight = float, int NAlign = 4> struct AxisParam {
        ...
        for (int x = 0; x < srcwidth; x++) {
            ...
            tjs_uint32 color =
                (tjs_uint32)((color_element[0] > 255) ? 255
                             : (color_element[0] < 0) ? 0
                                                      : color_element[0]);
            color += (tjs_uint32)((color_element[1] > 255) ? 255
                                  : (color_element[1] < 0) ? 0
                                                           : color_element[1])
                     << 8;
            color += (tjs_uint32)((color_element[2] > 255) ? 255
                                  : (color_element[2] < 0) ? 0
                                                           : color_element[2])
                     << 16;
            color += (tjs_uint32)((color_element[3] > 255) ? 255
                                  : (color_element[3] < 0) ? 0
                                                           : color_element[3])
                     << 24;
            *dstbits = color;
        ...
        if (blendfunc == nullptr) {
            ...
            for (int y = clip.offsety_; y < clip.height_; y++) {
                samplingVertical(y, workbits, dstheight, srcwidth, src, srcrect,
                                 wstarty);
                samplingHorizontal(
                    midbits, clip.offsetx_, clip.width_,
                    workbits); // 一時バッファにまずコピー, 範囲外は処理しない
...
void TVPResampleImage(const tTVPRect &cliprect, iTVPBaseBitmap *dest,
    ...
    try {
...
#endif
        {
            ...
            switch (type) {
            case stLinear:
                TVPWeightResample<BilinearWeight>(clip, func, dest, destrect,
                                                  src, srcrect);
                break;
            case stCubic:
                ...
                break;
            case stLanczos2:
                TVPWeightResample<LanczosWeight<2>>(clip, func, dest, destrect,
                                                    src, srcrect);
                break;
            case stLanczos3:
                TVPWeightResample<LanczosWeight<3>>(clip, func, dest, destrect,
                                                    src, srcrect);
                break;
            case stSpline16:
                TVPWeightResample<Spline16Weight>(clip, func, dest, destrect,
                                                  src, srcrect);
                break;
            case stSpline36:
                TVPWeightResample<Spline36Weight>(clip, func, dest, destrect,
                                                  src, srcrect);
                break;
            case stAreaAvg:
                ...
                break;
            case stGaussian:
                TVPWeightResample<GaussianWeight>(clip, func, dest, destrect,
                                                  src, srcrect);
                break;
            case stBlackmanSinc:
                TVPWeightResample<BlackmanSincWeight>(clip, func, dest,
