...
public:
    tTJSObjectProxy(){
        ...
        //		Dispatch1 = nullptr;
        //		Dispatch2 = nullptr;
        // Dispatch1 and Dispatch2 are to be set by subsequent call of
        // SetObjects
    };
...
#define OBJ2 ((objthis) ? (objthis) : (Dispatch2))
    ...
    tjs_error TJS_INTF_METHOD FuncCallByNum(tjs_uint32 flag, tjs_int num,
        ...
        if (hr == TJS_E_MEMBERNOTFOUND && Dispatch1 != Dispatch2)
            return Dispatch2->FuncCallByNum(flag, num, result, numparams, param,
                                            OBJ2);
        return hr;
    }
    ...
    tjs_error TJS_INTF_METHOD CreateNewByNum(tjs_uint32 flag, tjs_int num,
                                             ...
                                             tTJSVariant **param,
                                             iTJSDispatch2 *objthis) {
        tjs_error hr = Dispatch1->CreateNewByNum(flag, num, result, numparams,
                                                 param, OBJ1);
        if (hr == TJS_E_MEMBERNOTFOUND && Dispatch1 != Dispatch2)
    ...
    tjs_error TJS_INTF_METHOD IsInstanceOf(tjs_uint32 flag,
        ...
        if (hr == TJS_E_MEMBERNOTFOUND && Dispatch1 != Dispatch2)
            return Dispatch2->IsInstanceOf(flag, membername, hint, classname,
                                           OBJ2);
        return hr;
    }
    ...
    tjs_error TJS_INTF_METHOD Operation(tjs_uint32 flag,
        ...
        if (hr == TJS_E_MEMBERNOTFOUND && Dispatch1 != Dispatch2)
            return Dispatch2->Operation(flag, membername, hint, result, param,
                                        OBJ2);
        return hr;
    }
...
void tTJSVariantArrayStack::IncreaseVariantArray(tjs_int num) {
    ...
    if (NumArraysUsing > NumArraysAllocated) {
        Arrays = (tVariantArray *)TJS_realloc(Arrays, sizeof(tVariantArray) *
                                                          (NumArraysUsing));
        ...
        */
        ...
        try {
            ...
            if (numargs >= FuncDeclArgCount) {
                // given arguments are greater than or equal to desired
                // arguments
                if (FuncDeclArgCount) {
                    tTJSVariant *r = ra - 3;
                    tTJSVariant **a = args;
            ...
            if (FuncDeclCollapseBase >= 0) {
                ...
                *r = tTJSVariant(dsp, dsp);
                ...
                if (numargs > FuncDeclCollapseBase) {
                    ...
                    for (tjs_int c = 0, i = FuncDeclCollapseBase; i < numargs;
                         i++, c++)
...
#endif
            ra[-2].Clear(); // at least we must clear the object placed at local
                            // stack
...
#endif
        ra[-2]
            .Clear(); // at least we must clear the object placed at local stack
...
void tTJSInterCodeContext::DisplayExceptionGeneratedCode(
    tjs_int codepos, const tTJSVariant *ra) {
    ...
    if (val.Type() == tvtObject) {
        try {
            ...
            if (clo.Object != nullptr) {
                ...
                tjs_error hr =
                    clo.PropGet(0, message_name.c_str(), message_name.GetHint(),
                                &v2, nullptr);
                if (TJS_SUCCEEDED(hr)) {
...
#undef TJS_DEF_VM_P
            ...
            case VM_EVAL:
                Eval(TJS_GET_VM_REG(ra, code[1]),
                     TJSEvalOperatorIsOnGlobal ? nullptr
                                               : ra[-1].AsObjectNoAddRef(),
                     true);
                code += 2;
                break;
...
void tTJSInterCodeContext::GetPropertyDirect(tTJSVariant *ra,
    ...
    if (TJS_FAILED(hr))
        TJSThrowFrom_tjs_error(hr,
                               TJS_GET_VM_REG(DataArea, code[3]).GetString());
}
...
void tTJSInterCodeContext::SetPropertyDirect(tTJSVariant *ra,
    ...
    hr = clo.PropSetByVS(flags, name->AsStringNoAddRef(),
    ...
    if (TJS_FAILED(hr))
        TJSThrowFrom_tjs_error(hr,
                               TJS_GET_VM_REG(DataArea, code[2]).GetString());
}
...
void tTJSInterCodeContext::OperateProperty(tTJSVariant *ra,
    ...
    tjs_error hr;
    try {
        hr = clo.Operation(ope, nullptr, nullptr,
                           code[1] ? TJS_GET_VM_REG_ADDR(ra, code[1]) : nullptr,
                           TJS_GET_VM_REG_ADDR(ra, code[3]),
                           clo.ObjThis ? clo.ObjThis
                                       : ra[-1].AsObjectNoAddRef());
...
tjs_int tTJSInterCodeContext::CallFunctionDirect(tTJSVariant *ra,
    ...
    tTJSVariant *name = TJS_GET_VM_REG_ADDR(DataArea, code[3]);
    if (type == tvtString) {
        ProcessStringFunction(name->GetString(), TJS_GET_VM_REG(ra, code[2]),
                              pass_args, pass_args_count,
        ...
        ProcessOctetFunction(
            name->GetString(), TJS_GET_VM_REG(ra, code[2]).AsOctetNoAddRef(),
            pass_args, pass_args_count,
            code[1] ? TJS_GET_VM_REG_ADDR(ra, code[1]) : nullptr);
        hr = TJS_S_OK;
    } else {
...
#define TJS_STR_METHOD_IS(_name)                                               \
    ...
    if (TJS_STR_METHOD_IS(charAt)) {
        ...
        clo.FuncCall(0, replace_name.c_str(), replace_name.GetHint(), result, 2,
                     params, nullptr);
...
tjs_error TJS_INTF_METHOD tTJSInterCodeContext::FuncCall(
    ...
    if (membername == nullptr) {
        switch (ContextType) {
        case ctTopLevel:
            ExecuteAsFunction(objthis ? objthis
                                      : Block->GetTJS()->GetGlobalNoAddRef(),
                              nullptr, 0, result, 0);
            break;
...
//---------------------------------------------------------------------------
tjs_error TJS_INTF_METHOD tTJSInterCodeContext::PropGet(
    tjs_uint32 flag, const tjs_char *membername, tjs_uint32 *hint,
...
//---------------------------------------------------------------------------
tjs_error TJS_INTF_METHOD tTJSInterCodeContext::PropSet(
    tjs_uint32 flag, const tjs_char *membername, tjs_uint32 *hint,
    ...
    if (membername == nullptr) {
        if (ContextType == ctProperty) {
            ...
            if (PropSetter)
                return PropSetter->FuncCall(0, nullptr, nullptr, nullptr, 1,
    ...
    tjs_error hr;
    if (membername != nullptr && ContextType == ctClass && SuperClassGetter) {
...
//---------------------------------------------------------------------------
tjs_error TJS_INTF_METHOD tTJSInterCodeContext::CreateNew(
    tjs_uint32 flag, const tjs_char *membername, tjs_uint32 *hint,
...
//---------------------------------------------------------------------------
tjs_error TJS_INTF_METHOD tTJSInterCodeContext::IsInstanceOf(
    tjs_uint32 flag, const tjs_char *membername, tjs_uint32 *hint,
...
//---------------------------------------------------------------------------
tjs_error TJS_INTF_METHOD tTJSInterCodeContext::Operation(
    tjs_uint32 flag, const tjs_char *membername, tjs_uint32 *hint,
