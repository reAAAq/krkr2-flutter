...
public:
    ...
    MatrixClass() {
        this->_gpStatus = GdipCreateMatrix(&_gpMatrix);
    }

    MatrixClass(const GpRectF &rect, const GpPointF &point) {
        this->_gpStatus = GdipCreateMatrix3(
                &rect, &point, &_gpMatrix
        );
    }

    MatrixClass(float m11, float m12, float m21, float m22, float dx, float dy) {
        this->_gpStatus = GdipCreateMatrix2(
                m11, m12, m21, m22, dx, dy, &_gpMatrix
        );
    }
    ...
    GpStatus SetElements(float m11, float m12, float m21, float m22, float dx, float dy) {
        this->_gpStatus = GdipSetMatrixElements(
                this->_gpMatrix, m11, m12, m21, m22, dx, dy
        );
    ...
    GpStatus Multiply(MatrixClass *matrix, MatrixOrder order = MatrixOrderPrepend) {
        this->_gpStatus = GdipMultiplyMatrix(
                this->_gpMatrix, matrix->_gpMatrix, order
        );
        return this->_gpStatus;
    }
    ...
    GpStatus Rotate(float angle, MatrixOrder order) {
        this->_gpStatus = GdipRotateMatrix(
                this->_gpMatrix, angle, order
        );
        return this->_gpStatus;
    }

    GpStatus Translate(float offsetX, float offsetY, MatrixOrder order) {
        this->_gpStatus = GdipTranslateMatrix(
                this->_gpMatrix, offsetX, offsetY, order
        );
        return this->_gpStatus;
    }
    ...
    GpStatus Scale(float scaleX, float scaleY, MatrixOrder order) {
        this->_gpStatus = GdipScaleMatrix(
                this->_gpMatrix, scaleX, scaleY, order
        );
        return this->_gpStatus;
    }

    GpStatus Shear(float shearX, float shearY, MatrixOrder order) {
        this->_gpStatus = GdipShearMatrix(
                this->_gpMatrix, shearX, shearY, order
        );
        return this->_gpStatus;
    }
...
public:
    ...
    ImageClass(GpImage *gpImage) {
        this->_gpImage = gpImage;
    }

    static ImageClass *FromFile(
            const WCHAR *filename, bool useEmbeddedColorManagement
    ) {
        return new ImageClass{filename, useEmbeddedColorManagement};
    }

    ImageClass(const WCHAR *stream, bool useEmbeddedColorManagement = false) {
        if (useEmbeddedColorManagement)
            this->_gpStatus = GdipLoadImageFromFileICM(
                    stream, &this->_gpImage
            );
        else
            this->_gpStatus = GdipLoadImageFromFile(
                    stream, &this->_gpImage
            );
    }
    ...
    GpStatus GetBounds(RectFClass *srcRect, Unit *srcUnit) const {
        this->_gpStatus = GdipGetImageBounds(
                this->_gpImage, srcRect, srcUnit
        );
        return this->_gpStatus;
    }
    ...
    [[nodiscard]] float GetHorizontalResolution() const {
        float hr;
        this->_gpStatus = GdipGetImageHorizontalResolution(
                this->_gpImage, &hr
        );
        return hr;
    }
    ...
    [[nodiscard]] float GetVerticalResolution() const {
        float vr;
        this->_gpStatus = GdipGetImageVerticalResolution(
                this->_gpImage, &vr
        );
        return vr;
    }
    ...
    GpStatus RotateFlip(RotateFlipType rotateFlipType) {
        this->_gpStatus = GdipImageRotateFlip(
                this->_gpImage, rotateFlipType
        );
        return this->_gpStatus;
    }

    ~ImageClass() {
        GdipDisposeImage(this->_gpImage);
    }
...
public:
    ...
    ~PrivateFontCollection() {
        GdipDeletePrivateFontCollection(&_gpFC);
    }
...
static const GUID GdipEncoderScanMethod = {
        0x3A4E2661, 0x3109, 0x4E56,
        {0x85, 0x36, 0x42, 0xC1, 0x56, 0xE7, 0xDC, 0xFA}
};

static const GUID GdipEncoderVersion = {
        0x24D18C76, 0x814A, 0x41A4,
        {0xBF, 0x53, 0x1C, 0x21, 0x9C, 0xCC, 0xF7, 0x97}
};
...
static const GUID GdipEncoderCompression = {
        0x0E09D739DU, 0x0CCD4U, 0x44EEU,
        {0x8E, 0x0BA, 0x3F, 0x0BF, 0x8B, 0x0E4, 0x0FC, 0x58}
};

static const GUID GdipEncoderColorDepth = {
        0x66087055U, 0x0AD66U, 0x4C7CU,
        {0x9A, 0x18, 0x38, 0x0A2, 0x31, 0x0B, 0x83, 0x37}
};

static const GUID GdipEncoderSaveFlag = {
        0x292266FCU, 0x0AC40U, 0x47BFU,
        {0x8C, 0x0FC, 0x0A8, 0x5B, 0x89, 0x0A6, 0x55, 0x0DE}
};

static const GUID GdipEncoderSaveAsCMYK = {
        0x0A219BBC9U, 0x0A9DU, 0x4005U,
        {0x0A3, 0x0EE, 0x3A, 0x42, 0x1B, 0x8B, 0x0B0, 0x6C}
};

static const GUID GdipEncoderImageItems = {
        0x63875E13U, 0x1F1DU, 0x45ABU,
        {0x91, 0x95, 0x0A2, 0x9B, 0x60, 0x66, 0x0A6, 0x50}
};

static const GUID GdipEncoderTransformation = {
        0x8D0EB2D1U, 0x0A58EU, 0x4EA8U,
        {0x0AA, 0x14, 0x10, 0x80, 0x74, 0x0B7, 0x0B6, 0x0F9}
};
static const GUID GdipEncoderQuality = {
        0x1D5BE4B5U, 0x0FA4AU, 0x452DU,
        {0x9C, 0x0DD, 0x5D, 0x0B3, 0x51, 0x5, 0x0E7, 0x0EB}
};

static const GUID GdipEncoderLuminanceTable = {
        0x0EDB33BCEU, 0x266U, 0x4A77U,
        {0x0B9, 0x4, 0x27, 0x21, 0x60, 0x99, 0x0E7, 0x17}
};

static const GUID GdipEncoderChrominanceTable = {
        0x0F2E455DCU, 0x9B3U, 0x4316U,
        {0x82, 0x60, 0x67, 0x6A, 0x0DA, 0x32, 0x48, 0x1C}
};


static const CLSID bmpEncoderClsid = {0x557cf400, 0x1a04, 0x11d3,
                                      {0x9a, 0x73, 0x0, 0x0, 0xf8, 0x1e, 0xf3, 0x2e}};
static const CLSID tifEncoderClsid = {0x557cf405, 0x1a04, 0x11d3,
                                      {0x9a, 0x73, 0x0, 0x0, 0xf8, 0x1e, 0xf3, 0x2e}};
static const CLSID gifEncoderClsid = {0x557cf402, 0x1a04, 0x11d3,
                                      {0x9a, 0x73, 0x0, 0x0, 0xf8, 0x1e, 0xf3, 0x2e}};
static const CLSID pngEncoderClsid = {0x557cf406, 0x1a04, 0x11d3,
                                      {0x9a, 0x73, 0x0, 0x0, 0xf8, 0x1e, 0xf3, 0x2e}};
static const CLSID jpegEncoderClsid = {0x557cf401, 0x1a04, 0x11d3,
                                       {0x9a, 0x73, 0x0, 0x0, 0xf8, 0x1e, 0xf3, 0x2e}};
static const CLSID icoEncoderClsid = {0x557cf407, 0x1a04, 0x11d3,
                                      {0x9a, 0x73, 0x0, 0x0, 0xf8, 0x1e, 0xf3, 0x2e}};
static const CLSID wmfEncoderClsid = {0x557cf404, 0x1a04, 0x11d3,
                                      {0x9a, 0x73, 0x0, 0x0, 0xf8, 0x1e, 0xf3, 0x2e}};
static const CLSID emfEncoderClsid = {0x557cf403, 0x1a04, 0x11d3,
                                      {0x9a, 0x73, 0x0, 0x0, 0xf8, 0x1e, 0xf3, 0x2e}};
