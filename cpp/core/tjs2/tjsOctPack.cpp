...
enum OctPackType {
    ...
    OctPack_noshort, // n : short���l(�l�b�g���[�N�o�C�g�I�[�_) network byte
                     // order short
    OctPack_NOLONG, // N : long���l(�l�b�g���[�N�o�C�g�I�[�_) network byte order
                    // long
...
// mtol
static void HexToBin(std::vector<tjs_uint8> &bin, const ttstr &arg, bool mtol,
                     tjs_int len) {
    const tjs_char *str = arg.c_str();
    if (len < 0)
        len = arg.length();
...
static void ReadNumberLE(std::vector<tjs_uint8> &result,
                         const std::vector<tTJSVariant> &args, tjs_int numargs,
                         tjs_int &argindex, tjs_int len) {
    if (len < 0)
...
static void encodeBase64(const tjs_uint8 *inbuf, tjs_uint insize,
    ...
    for (i = 0; i <= insize_3; i += 3) {
        ...
        outbuf.push_back(
            base64str[((inbuf[i] << 4) & 0x30) | ((inbuf[i + 1] >> 4) & 0x0F)]);
...
static void decodeBase64(const ttstr &inbuf, std::vector<tjs_uint8> &outbuf) {
    ...
    while (dptr < len_4) {
        ...
        dptr++;
        outbuf.push_back(static_cast<tjs_uint8>((base64tonum[data[dptr]] << 6) |
                                                (base64tonum[data[dptr + 1]])));
        dptr += 2;
    }
    outbuf.push_back(static_cast<tjs_uint8>(
        (base64tonum[data[dptr]] << 2) | (base64tonum[data[dptr + 1]] >> 4)));
    dptr++;
...
static tTJSVariantOctet *Pack(const std::vector<OctPackTemplate> &templ,
    ...
    for (tjs_size i = 0; i < count && argindex < numargs; argindex++) {
        ...
        case OctPack_char: // c : �����t��1�o�C�g���l(-128 �` 127)
            ReadNumber<tjs_int8, tjs_int, 1, tjs_int8>(result, args, numargs,
                                                       argindex, len);
            break;
        case OctPack_CHAR: // C : ��������1�o�C�g���l(0�`255)
            ReadNumber<tjs_uint8, tjs_int, 1, tjs_uint8>(result, args, numargs,
                                                         argindex, len);
            break;
        case OctPack_double: // d : �{���x���������_
            ReadNumber<tjs_real, tjs_real, 8, tjs_uint64>(result, args, numargs,
                                                          argindex, len);
            break;
        case OctPack_float: // f : �P���x���������_
            ReadNumber<float, tjs_real, 4, tjs_uint32>(result, args, numargs,
                                                       argindex, len);
            break;
        ...
        case OctPack_long: // l : �����t��long���l(�ʏ�4�o�C�g)
            ReadNumber<tjs_int, tjs_int, 4, tjs_int32>(result, args, numargs,
                                                       argindex, len);
            break;
        ...
        case OctPack_SHORT: // S : ��������short���l(�ʏ�2�o�C�g) unsign
            ReadNumber<tjs_uint16, tjs_int, 2, tjs_uint16>(
                result, args, numargs, argindex, len);
            break;
        case OctPack_leshort: // v : ���g���G���f�B�A���ɂ��unsigned short�l
                              // little endian short
            ReadNumberLE<tjs_uint16, tjs_int, 2, tjs_uint16>(
                result, args, numargs, argindex, len);
            break;
        case OctPack_LELONG: // V : ���g���G���f�B�A���ɂ��unsigned long�l little
                             // endian long
...
static void BinToNumberLEReal(std::vector<TRet> &result, const tjs_uint8 *data,
                              const tjs_uint8 *tail, tjs_int len) {
    if (len < 0)
        len = (tjs_uint)(((tail - data) + NBYTE - 1) / NBYTE);
    if ((data + len * NBYTE) < tail)
...
// mtol
static void BinToHex(const tjs_uint8 *data, const tjs_uint8 *tail, tjs_uint len,
                     ttstr &result, bool mtol) {
    if ((data + len) < tail)
        tail = data + (len + 1) / 2;
    // std::vector<tjs_char> tmp(len+1);
    std::vector<tjs_char> tmp;
...
static iTJSDispatch2 *Unpack(const std::vector<OctPackTemplate> &templ,
    ...
    tTJSArrayNI *ni;
    if (TJS_FAILED(result->NativeInstanceSupport(TJS_NIS_GETINSTANCE,
                                                 TJSGetArrayClassID(),
    ...
    for (tjs_int i = 0; i < (tjs_int)count && current < tail; argindex++) {
        ...
        case OctPack_char: { // c : �����t��1�o�C�g���l(-128 �` 127)
            ...
            for (std::vector<tjs_int8>::const_iterator iter = ret.begin();
                 iter != ret.end(); iter++) {
        ...
        case OctPack_CHAR: { // C : ��������1�o�C�g���l(0�`255)
            ...
            for (std::vector<tjs_uint8>::const_iterator iter = ret.begin();
                 iter != ret.end(); iter++) {
                result->Add(ni, tTJSVariant((tjs_int)*iter));
            }
            current += len;
        ...
        case OctPack_double: { // d : �{���x���������_
            ...
            BinToReal<tjs_real, tjs_uint64, 8>(ret, current, tail,
                                               (tjs_uint)len);
            for (std::vector<tjs_real>::const_iterator iter = ret.begin();
                 iter != ret.end(); iter++) {
        ...
        case OctPack_LONG: { // L : ��������long���l(�ʏ�4�o�C�g)
            ...
            for (std::vector<tjs_uint>::const_iterator iter = ret.begin();
                 iter != ret.end(); iter++) {
        ...
        case OctPack_noshort: { // n : unsigned
            ...
            for (std::vector<tjs_uint16>::const_iterator iter = ret.begin();
                 iter != ret.end(); iter++) {
                result->Add(ni, tTJSVariant((tjs_int)*iter));
            }
            current += len * 2;
        ...
        case OctPack_NOLONG: { // N : unsigned
                               // long���l(�l�b�g���[�N�o�C�g�I�[�_) network
                               // byte order long
            if (len < 0)
                len = ((tail - current) / 4);
            std::vector<tjs_uint> ret;
        ...
        case OctPack_short: { // s : �����t��short���l(�ʏ�2�o�C�g) sign
            ...
            for (std::vector<tjs_int16>::const_iterator iter = ret.begin();
                 iter != ret.end(); iter++) {
                result->Add(ni, tTJSVariant((tjs_int)*iter));
            }
            current += len * 2;
        ...
        case OctPack_LELONG: { // V : ���g���G���f�B�A���ɂ��unsigned long�l
                               // little endian long
            if (len < 0)
                len = ((tail - current) / 4);
            std::vector<tjs_uint> ret;
    ...
    if (result) {
        ...
        *result = oct;
        if (oct)
            ...
            *result =
                tTJSVariant((iTJSDispatch2 *)nullptr, (iTJSDispatch2 *)nullptr);
    }
    return TJS_S_OK;
}
