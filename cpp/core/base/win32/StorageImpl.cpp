...
//---------------------------------------------------------------------------
tTJSBinaryStream *TJS_INTF_METHOD tTVPFileMedia::Open(const ttstr &name,
                                                      tjs_uint32 flags) {
...
//---------------------------------------------------------------------------
void TJS_INTF_METHOD tTVPFileMedia::GetListAt(const ttstr &_name,
                                              iTVPStorageLister *lister) {
    ttstr name(_name);
    GetLocalName(name);
#if 0
...
void TJS_INTF_METHOD tTVPFileMedia::GetLocallyAccessibleName(ttstr &name) {
    ...
    if (TJS_strncmp(ptr, TJS_W("./"), 2)) {
        ...
        // AFAIK 32-bit version of Windows assumes that '/' can be used as a
        // path delimiter. Can UNC "\\\\" be replaced by "//" though ?
...
static tTVPArchive *(*ArchiveCreators[])(const ttstr &name,
                                         tTJSBinaryStream *st,
                                         bool normalizeFileName) = {
    TVPOpenZIPArchive, TVPOpen7ZArchive, TVPOpenTARArchive,
...
//---------------------------------------------------------------------------
tjs_uint64 TJS_INTF_METHOD tTVPLocalFileStream::Seek(tjs_int64 offset,
                                                     tjs_int whence) {
    if (MemBuffer) {
...
//---------------------------------------------------------------------------
tjs_uint TJS_INTF_METHOD tTVPLocalFileStream::Read(void *buffer,
                                                   tjs_uint read_size) {
    if (MemBuffer) {
...
//---------------------------------------------------------------------------
tjs_uint TJS_INTF_METHOD tTVPLocalFileStream::Write(const void *buffer,
                                                    tjs_uint write_size) {
    if (MemBuffer) {
...
//---------------------------------------------------------------------------
tjs_uint64 TJS_INTF_METHOD tTVPBinaryStreamAdapter::Seek(tjs_int64 offset,
                                                         tjs_int whence) {
    DWORD origin;
...
//---------------------------------------------------------------------------
tjs_uint TJS_INTF_METHOD tTVPBinaryStreamAdapter::Read(void *buffer,
                                                       tjs_uint read_size) {
    ULONG cb = read_size;
    ULONG read;
...
//---------------------------------------------------------------------------
tjs_uint TJS_INTF_METHOD tTVPBinaryStreamAdapter::Write(const void *buffer,
                                                        tjs_uint write_size) {
    ULONG cb = write_size;
    ULONG written;
...
tTVPPluginHolder::tTVPPluginHolder(const ttstr &aname) {
    ...
    if (!place.IsEmpty()) {
        ...
        // not found in TVP storage system; search exepath, exepath\system,
        // exepath\plugin
#if 0
        ttstr exepath =
