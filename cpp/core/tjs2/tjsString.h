...
/*start-of-tTJSString*/
    ...
    public:
        ...
        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString,
                       (const tjs_char *str)) { Ptr = TJSAllocVariantString(str); }

        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString,
                       (const tjs_nchar *str)) { Ptr = TJSAllocVariantString(str); }
        ...
        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString,
                       (const tTJSString &str, int n)) // construct with first n chars of str
        { Ptr = TJSAllocVariantString(str.c_str(), n); }

        TJS_METHOD_DEF(TJS_METHOD_RET_EMPTY, tTJSString,
                       (const tjs_char *str, int n)) // same as above except for str's type
        ...
        [[nodiscard]] std::wstring c_wstr() const {
            ...
            std::wstring result = conv::to_utf<wchar_t>(
                    AsNarrowStdString(), "UTF-8"
            );
            return result;
        }
        ...
        [[nodiscard]] const std::string AsStdString() const {
...
#ifdef __CODEGUARD__
            if(!Ptr) return 0; // tTJSVariantString::GetLength can return zero if 'this' is nullptr
#endif
