...
template <typename T> class tRefHolder {
...
template <typename T> class tVectorList {
protected:
    ...
    inline void _ensureCapacity() {
        if (UnusedIndexStack.empty()) {
            ...
            if (!PointerBuffPtr) {
                ...
                PointerBuffPtr =
                    (_tVectorList_Node *)new char[VECLIST_MIN_CAPACITY *
                ...
                _tVectorList_Node *newPtr =
                    (_tVectorList_Node
                         *)new char[nNewCapacity * sizeof(_tVectorList_Node)];
