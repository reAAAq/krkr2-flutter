...
TVP_MSG_DEFINE(
    TVPInfoLoadingExecutableEmbeddedOptionsFailed,
...
TVP_MSG_DEFINE(
    TVPCpuCheckFailureCpuFamilyOrLesserIsNotSupported,
...
TVP_MSG_DEFINE(
    TVPBasicDrawDeviceFailedToCreateDirect3DDevicesUnknownReason,
...
TVP_MSG_DEFINE(
    TVPBasicDrawDeviceFailedToCreateDirect3DDeviceUnknownReason,
...
TVP_MSG_DEFINE(
    TVPRoughVsyncIntervalStillSeemsWrong,
...
TVP_MSG_DEFINE(
    TVPInfoNotFoundScreenModeFromDriver,
    "(info) Panic! There is no reasonable candidate screen mode provided from "
    "the driver ... trying to use the default screen size and color depth ...")
...
TVP_MSG_DEFINE(TVPD3dErrMoreData, "D3D : There is more data available than the "
...
TVP_MSG_DEFINE(
    TVPExceptionDatatypeMisalignment,
    "Data Type Misalignment: The thread attempts to read or write data that is "
    "misaligned on hardware that does not provide alignment. For example, "
    "16-bit values must be aligned on 2-byte boundaries,) 32-bit values on "
...
TVP_MSG_DEFINE(
    TVPExceptionArrayBoundsExceeded,
    "Array Bounds Exceede: The thread attempts to access an array element that "
...
TVP_MSG_DEFINE(
    TVPExceptionIntOverflow,
    "Overflow: The result of an integer operation creates a value that is too "
    "large to be held by the destination register. In some cases, this will "
    "result in a carry out of the most significant bit of the result. Some "
    "operations do not set the carry flag.")
TVP_MSG_DEFINE(
    TVPExceptionPrivInstruction,
    "Priv Instruction: The thread attempts to execute an instruction with an "
...
TVP_MSG_DEFINE(TVPExceptionGuardPage, "Guard Page: The thread accessed memory "
                                      "allocated with the PAGE_GUARD modifier.")
TVP_MSG_DEFINE(
    TVPExceptionIllegalInstruction,
...
TVP_MSG_DEFINE(
    TVPExceptionInvalidDisposition,
    "Invalid Disposition: An exception handler returns an invalid disposition "
    "to the exception dispatcher. Programmers using a high-level language such "
