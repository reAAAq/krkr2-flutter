...
struct DVDVideoPicture {
    double pts; // timestamp in seconds, used in the CVideoPlayer class to keep
                // track of pts
    double dts;
...
public:
     ...
     * pData and iSize can be nullptr, this means we should flush the rest of
     * the data.
     */
    ...
    /**
     * will be called by video player indicating if a frame will eventually be
     * dropped codec can then skip actually decoding the data, just consume the
     * data set picture headers
     */
    virtual void SetDropState(bool bDrop) = 0;

    /**
     * will be called by video player indicating the playback speed. see
     * DVD_PLAYSPEED_NORMAL, DVD_PLAYSPEED_PAUSE and friends.
     */
     ...
     * information.
     * - pts : right after decoder, used to detect gaps (dropped frames in
     * decoder)
     * - droppedFrames : indicates if decoder has dropped a frame
     *                 -1 means that decoder has no info on this.
     * - skippedPics : indicates if postproc has skipped a already decoded
     * picture -1 means that decoder has no info on this.
     *
     * If codec does not implement this method, pts of decoded frame at input
     * video player is used. In case decoder does post-proc and de-interlacing
     ...
     *                  to be finished instead of returning empty and getting
     * another packet.
     *
     * DVD_CODEC_CTRL_DRAIN :
     *                  instruct decoder to deliver last pictures without
     * requesting new packets
     *
     * DVD_CODEC_CTRL_DROP :
     *                  this packet is going to be dropped. decoder is free to
     * use it for decoding
     *
     */
